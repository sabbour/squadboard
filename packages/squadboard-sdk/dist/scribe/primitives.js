 ...
 */
export async function archiveDecisionsBySize(decisionsPath, opts = {}) {
 ...
 */
export async function mergeInbox(inboxDir, decisionsPath) {
    let files;
    try {
    ...
    const existingHeadings = new Set([...existing.matchAll(/^## (.+)$/gm)].map(([, h]) => h.trim().toLowerCase()));
    const chunks = [];
    for (const f of files) {
        ...
        if (firstHeading)
            existingHeadings.add(firstHeading[1].trim().toLowerCase());
        await unlink(f).catch(() => { });
    }
    if (chunks.length > 0) {
 ...
 */
export async function writeOrchestrationLogs(manifest, logsDir, datetime) {
 ...
 */
export async function writeSessionLog(manifest, logsDir, datetime) {
    const ts = datetime.replace(/[:.]/g, '-').replace(/[TZ]/g, (c) => (c === 'T' ? 'T' : 'Z'));
 ...
 */
export async function crossAgentHistoryUpdates(manifest, agentsDir) {
    const updated = [];
 ...
 */
export async function summarizeHistoryIfLarge(historyPath, thresholdBytes = 15_360) {
    ...
    const learningsBody = nextH2 === -1 ? learningsAndRest : learningsAndRest.slice(0, nextH2);
 ...
 */
export async function commitScribeFiles(allowedPaths, message, repoRoot) {
