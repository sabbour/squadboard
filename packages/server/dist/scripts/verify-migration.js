        ...
        http.get(`${SERVER_BASE}/api/system/db-counts`, (res) => {
            ...
            res.on('end', () => {
                try {
                    ...
                    if (parsed.ok)
                        resolve(parsed.counts);
                    else
                        reject(new Error(`Server db-counts returned ok=false: ${body}`));
                }
                catch (e) {
                    reject(e);
                }
...
export async function runVerify(opts = {}) {
    ...
    if (serverLive) {
        ...
        if (!pglite)
            throw new Error('[verify] PGlite instance null');
    ...
    for (const r of results) {
        ...
        console.log(`${r.table.padEnd(maxTable)}  ${String(r.expected).padStart(6)}  ${String(r.actual).padStart(5)}  ${status}`);
    }
    console.log('');
    if (allOk) {
