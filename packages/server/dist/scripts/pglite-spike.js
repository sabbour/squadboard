...
async function main() {
    ...
    try {
        const r = await db.query(`SELECT EXISTS(SELECT 1 FROM information_schema.tables WHERE table_name = 'projects') AS exists`);
        if (!r.rows[0]?.exists)
            throw new Error('projects table not found via information_schema');
    ...
    try {
        const r = await db.query(`SELECT EXISTS(SELECT 1 FROM pg_type WHERE typname = 'column_status') AS exists`);
        // column_status should exist (we created it above)
        if (!r.rows[0]?.exists)
            throw new Error('column_status enum not found via pg_type');
    ...
    `);
      ...
      DO UPDATE SET
        ...
        updated_at = NOW();
    `);
        const r = await db.query(`SELECT name FROM review_policy_presets WHERE slug='solo'`);
        if (r.rows[0]?.name !== 'Solo Reviewer v2')
            throw new Error(`Expected 'Solo Reviewer v2', got '${r.rows[0]?.name}'`);
    ...
    try {
        const r = await db.query(`INSERT INTO projects (name, path) VALUES ('spike-test', '/tmp') RETURNING id`);
        const id = r.rows[0]?.id;
        if (!id || id.length !== 36)
            throw new Error(`UUID default produced unexpected value: ${id}`);
    ...
    try {
        const projResult = await db.query(`INSERT INTO projects (name, path) VALUES ('cascade-test', '/foo') RETURNING id`);
        const projectId = projResult.rows[0].id;
        // Insert 3 issues
        for (let i = 1; i <= 3; i++) {
            await db.query(`INSERT INTO issues (project_id, title, body) VALUES ($1, $2, 'body') RETURNING id`, [projectId, `Issue ${i}`]);
        }
        // Verify count
        const countResult = await db.query(`SELECT COUNT(*) AS cnt FROM issues WHERE project_id = $1`, [projectId]);
        const cnt = parseInt(countResult.rows[0].cnt, 10);
        if (cnt !== 3)
            throw new Error(`Expected 3 issues, got ${cnt}`);
        // Update one
        await db.query(`UPDATE issues SET title = 'Updated Issue 1' WHERE project_id = $1 AND title = 'Issue 1'`, [projectId]);
        // Delete project — cascade must clear issues
        await db.query(`DELETE FROM projects WHERE id = $1`, [projectId]);
        const afterCascade = await db.query(`SELECT COUNT(*) AS cnt FROM issues WHERE project_id = $1`, [projectId]);
        const remaining = parseInt(afterCascade.rows[0].cnt, 10);
        if (remaining !== 0)
            throw new Error(`CASCADE DELETE failed: ${remaining} issue(s) remain`);
    ...
    try {
        const r = await db.query(`INSERT INTO projects (name, path) VALUES ($1, $2) RETURNING id`, ['param-test', '/bar']);
        const id = r.rows[0]?.id;
        if (!id)
            throw new Error('No id returned from parameterized INSERT');
    ...
    try {
        const r = await db.query(`INSERT INTO projects (name, path) VALUES ($1, $2) RETURNING created_at`, ['ts-test', '/ts']);
        const ts = r.rows[0]?.created_at;
        if (!ts)
            throw new Error('No created_at returned');
    ...
    try {
        ...
        const affected = upd.affectedRows;
        if (affected !== 1)
            throw new Error(`Expected affectedRows=1, got ${affected}`);
...
//# sourceMappingURL=pglite-spike.js.map