 ...
 */
export function pruneBackups(dir, retainCount) {
    if (!existsSync(dir))
        return [];
    ...
    for (const f of toDelete) {
        try {
            ...
            console.warn(`[backup] Failed to prune ${f.path}: ${err.message}`);
        }
    }
    return pruned;
}
...
export async function runBackup(opts = {}) {
    ...
    if (!pglite) {
        ...
        pglite = getPglite();
        if (!pglite)
            throw new Error('[backup] PGlite instance null after start');
    }
    // ── Dump ──────────────────────────────────────────────────────────────────
