 ...
 */
async function loadInFlightApproveSteps() {
    const db = getDb();
    ...
    const rows = await db
        ...
        .innerJoin(schema.workflowVersions, eq(schema.workflowVersions.id, schema.workflowRuns.workflowVersionId))
        .innerJoin(schema.workflows, eq(schema.workflows.id, schema.workflowVersions.workflowId))
        .where(and(eq(schema.stepRuns.stepType, 'approve'), inArray(schema.stepRuns.status, ['pending', 'running'])));
    return rows;
}
async function applyTimeoutAction(row, step, policy, deadline) {
    const db = getDb();
    ...
    switch (policy.timeout_action) {
        case 'notify': {
            ...
            if (alreadyNotified)
                return { fired: true, policy, deadline, mutated: false };
 ...
 */
export async function sweepReviewTimeouts() {
    const result = { scanned: 0, fired: 0, errors: 0 };
    let rows;
    try {
    ...
    const projectDefaultCache = new Map();
        ...
        try {
            ...
            if (definitionCache.has(row.workflowVersionId)) {
                ...
                try {
                    ...
                    console.warn(`[review-timeout] failed to parse workflow_version ${row.workflowVersionId}:`, e);
                    definition = null;
                }
            ...
            if (!stepDef || stepDef.type !== 'approve')
                continue;
            ...
            const { policy } = resolvePolicyForStep(stepDef, {
                project: projectDefault,
            });
            const timeoutMs = parseDurationMs(policy.timeout);
            if (!timeoutMs) {
                console.warn(`[review-timeout] step_run ${row.stepRun.id} has unparseable timeout '${policy.timeout}' — skipping`);
                continue;
            }
            ...
            const startedAt = row.stepRun.updatedAt instanceof Date
                ...
                : new Date(row.stepRun.updatedAt).getTime();
            const deadline = new Date(startedAt + timeoutMs);
            if (deadline.getTime() > now)
                continue; // not yet expired
            const decision = await applyTimeoutAction(row, stepDef, policy, deadline);
            if (decision.fired)
                result.fired++;
        }
        catch (e) {
    ...
    if (result.fired > 0 || (result.errors > 0 && process.env.LOG_LEVEL === 'debug')) {
        console.log(`[review-timeout] swept ${result.scanned} step(s); fired ${result.fired}, errors ${result.errors}`);
    }
    return result;
}
//# sourceMappingURL=review-timeout-sweep.js.map