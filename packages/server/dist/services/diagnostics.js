...
export async function resolveSquadDir(storedPath) {
...
async function checkSdkModels() {
    ...
    try {
        ...
        let models;
        try {
            models = (await withTimeout(client.listModels(), 10_000));
        }
        finally {
            await client.disconnect().catch(() => { });
        }
...
async function checkSquadDirShape(projectId) {
    const start = Date.now();
    try {
        ...
        if (!resolved.ok) {
            return {
                ...
                remediation: 'Update the project record so `path` points to either the project root (containing .squad/) or the .squad/ directory itself.',
                durationMs: Date.now() - start,
            };
        }
...
async function checkMcpServers(projectId) {
    const start = Date.now();
    try {
        const db = getDb();
        ...
        await Promise.allSettled(rows.map(async (row) => {
            ...
            try {
                ...
                if (!res.ok)
                    unreachable.push(`${row.name} (HTTP ${res.status})`);
            }
            catch {
...
async function checkMcpServersGlobal() {
    const start = Date.now();
    return {
        id: 'mcp.servers',
        label: 'MCP servers reachable',
...
async function checkDiskWriteable(projectId) {
    const start = Date.now();
    const targets = [
    ...
    for (const target of targets) {
        ...
        try {
            ...
            failures.push(`${target.label}: ${err instanceof Error ? err.message : String(err)}`);
            // Best-effort cleanup
