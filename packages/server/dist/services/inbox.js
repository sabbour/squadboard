...
import { resolveModel } from '../sdk/model-defaults.js';
// ---------------------------------------------------------------------------
...
export async function listInboxItems(filters = {}) {
    ...
    const conds = [];
    if (filters.userId)
        conds.push(eq(schema.inboxItems.userId, filters.userId));
    if (filters.status)
        conds.push(eq(schema.inboxItems.status, filters.status));
    if (filters.projectId)
        conds.push(eq(schema.inboxItems.suggestedProjectId, filters.projectId));
    let q = db
        .select()
...
export async function updateInboxItem(id, patch) {
    ...
    if (patch.formulatedTitle !== undefined)
        update.formulatedTitle = patch.formulatedTitle;
    if (patch.formulatedBody !== undefined)
        update.formulatedBody = patch.formulatedBody;
    if (patch.suggestedLabels !== undefined)
        update.suggestedLabels = patch.suggestedLabels;
    if ('suggestedProjectId' in patch)
        update.suggestedProjectId = patch.suggestedProjectId ?? null;
    if ('suggestedColumn' in patch)
        update.suggestedColumn = patch.suggestedColumn ?? null;
    if ('confidence' in patch)
        update.confidence = patch.confidence ?? null;
    if ('rationale' in patch)
        update.rationale = patch.rationale ?? null;
...
export async function discardInboxItem(id) {
    const db = getDb();
    const existing = await getInboxItem(id);
    if (!existing) {
        throw Object.assign(new Error('inbox item not found'), { status: 404 });
...
// ---------------------------------------------------------------------------
export async function publishInboxItem(id, input) {
    ...
    const column = input.columnSlug ?? 'backlog';
    const validCols = ['backlog', 'todo', 'in_progress', 'in_review', 'done'];
...
async function buildPrompt(item) {
    ...
    const projectsList = projects.length
        ? projects
            ...
            .join('\n')
        : '(no projects yet)';
    let labelProjectId = item.suggestedProjectId ?? null;
...
function normalizePayload(parsed, validProjectIds) {
    ...
    const suggestedColumn = VALID_COLUMNS.includes(colRaw)
        ? colRaw
        : 'backlog';
    const confRaw = typeof p.confidence === 'string' ? p.confidence : 'medium';
    const confidence = (VALID_CONFIDENCE.has(confRaw) ? confRaw : 'medium');
 ...
 */
async function callFormulator(prompt, model) {
...
export async function formulateInboxItem(id) {
    const item = await getInboxItem(id);
    if (!item) {
        throw Object.assign(new Error('inbox item not found'), { status: 404 });
    }
    ...
    if (now - last < FORMULATE_THROTTLE_MS) {
        throw new ThrottledError(`formulate throttled — try again in ${Math.ceil((FORMULATE_THROTTLE_MS - (now - last)) / 1000)}s`);
    }
