 ...
 */
export function previewNextFireTimes(cronExpr, count = 3, timezone = 'UTC', fromDate = new Date()) {
 ...
 */
export function computeNextFire(cronExpr, timezone = 'UTC', from = new Date()) {
    const interval = CronExpressionParser.parse(cronExpr, {
        currentDate: from,
        tz: timezone,
    });
...
// ---------------------------------------------------------------------------
export async function recomputeNextFire(scheduleId) {
    const db = getDb();
    const [row] = await db
        .select()
...
export async function enableSchedule(scheduleId) {
    const db = getDb();
    await db
        .update(schema.ceremonySchedules)
...
export async function spawnCeremonyRun(workflowId, opts = { trigger: 'manual' }) {
    ...
    const versions = await db
        ...
        .where(and(eq(schema.workflowVersions.workflowId, workflowId), eq(schema.workflowVersions.isActive, true)))
        .limit(1);
    const activeVersion = versions[0];
    if (!activeVersion) {
        console.warn(`[ceremony] spawn skipped — workflow ${workflowId} has no active version`);
        return null;
    }
    const anchorIssueId = opts.anchorIssueId ?? (await resolveAnchorIssue(workflow.projectId));
    if (!anchorIssueId) {
        console.warn(`[ceremony] spawn skipped — workflow ${workflowId} has no anchor issue ` +
...
// ---------------------------------------------------------------------------
export async function sweepDueSchedules(now = new Date()) {
    const db = getDb();
    const result = { scanned: 0, fired: 0, skipped: 0, errors: 0 };
    const due = await db
        .select()
        .from(schema.ceremonySchedules)
        .where(and(eq(schema.ceremonySchedules.enabled, true), lte(schema.ceremonySchedules.nextFireAt, now)));
    ...
    for (const sched of due) {
        try {
            ...
            const updated = await db
                ...
                .where(and(eq(schema.ceremonySchedules.id, sched.id), lte(schema.ceremonySchedules.nextFireAt, now)))
            ...
            const nextFire = computeNextFire(sched.cronExpr, sched.timezone ?? 'UTC', now);
            await db
                .update(schema.ceremonySchedules)
                .set({
                lastFiredAt: now,
