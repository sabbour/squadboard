...
function buildAgentPrompt(draft, existingNames) {
...
function normalizeAgentDraft(parsed, existingNames) {
    ...
    if (!name || !KEBAB_RE.test(name))
        throw new Error('LLM produced an invalid agent name');
    ...
    if (!role)
        throw new Error('LLM payload missing `role`');
...
export async function formulateAgentDraft(projectId, draft, existingAgentNames) {
    ...
    console.log(`[agents] formulating draft (${trimmed.length} chars) with model=${modelUsed.model} (via ${modelUsed.via})`);
...
function buildTeamPrompt(draft, universes) {
...
function normalizeTeamDraft(parsed) {
    if (!parsed || typeof parsed !== 'object') {
        throw new Error('LLM payload was not a JSON object');
    }
    const p = parsed;
...
export async function formulateTeamDraft(projectId, draft) {
    const trimmed = (draft ?? '').trim();
    if (!trimmed) {
        throw Object.assign(new Error('draft is required'), { status: 400 });
    }
    ...
    const { raw, modelUsed } = await runFormulator({ prompt, projectId });
    console.log(`[team] formulating draft (${trimmed.length} chars) with model=${modelUsed.model} (via ${modelUsed.via})`);
    const parsed = extractJsonObject(raw);
    ...
    if (!validIds.has(team.universe)) {
        team.universe = validIds.has('usual-suspects')
            ? 'usual-suspects'
            : universes[0]?.id ?? team.universe;
    }
    return { team, modelUsed };
}
//# sourceMappingURL=hire-formulator.js.map