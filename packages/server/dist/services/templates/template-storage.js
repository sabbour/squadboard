 ...
 */
export async function writeTemplateMirror(projectId, kind, payload, meta) {
    try {
        const db = getDb();
        ...
        console.warn(`[template-storage] mirror write failed for ${kind} template ${meta.id}: ${message}`);
        return { storagePath: null, error: message };
    }
}
//# sourceMappingURL=template-storage.js.map