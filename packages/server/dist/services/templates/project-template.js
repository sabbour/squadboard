...
export async function exportProject(projectId) {
    ...
    const agents = await Promise.all(agentRows.map(async (agent) => {
    ...
    const ceremonies = await Promise.all(ceremonyRows.map(async (c) => {
    ...
    const mcpServers = mcpRows.map((m) => ({
    ...
    const routingRules = routingRows.map((r) => ({
 ...
 */
export async function importProject(payload, newProjectName, newSquadPath) {
    const pool = getPool();
    ...
    `, [newProjectName, newSquadPath, payload.meta.defaultModel ?? null]);
      ...
      `, [projectId, a.name, a.role, a.model, a.status ?? 'active', charterPath, charterHash]);
            ...
            for (const key of a.skillKeys) {
                ...
                if (sid)
                    await client.query(`INSERT INTO agent_skills (agent_id, skill_id) VALUES ($1, $2) ON CONFLICT DO NOTHING`, [agentId, sid]);
            }
            for (const key of a.toolKeys) {
                const tid = toolIdByKey[key];
                if (tid)
                    await client.query(`INSERT INTO agent_tools (agent_id, tool_id) VALUES ($1, $2) ON CONFLICT DO NOTHING`, [agentId, tid]);
            }
            for (const name of a.mcpNames) {
                const mid = mcpIdByName[name];
                if (mid)
                    await client.query(`INSERT INTO agent_mcp_servers (agent_id, mcp_server_id) VALUES ($1, $2) ON CONFLICT DO NOTHING`, [agentId, mid]);
            }
        }
        // Ceremonies.
        for (const c of payload.ceremonies) {
            if (!c.yamlContent)
                continue; // skip ceremonies without content
...
export async function saveAsTemplate(projectId, name, description) {
    const pool = getPool();
...
// instantiateTemplate
// ---------------------------------------------------------------------------
export async function instantiateTemplate(templateId, newProjectName, newSquadPath) {
    const db = getDb();
    const [tmpl] = await db
        .select()
    ...
    if (tmpl.kind !== 'project')
        throw new Error(`Template ${templateId} is not a project template`);
    return importProject(tmpl.payload, newProjectName, newSquadPath);
}
