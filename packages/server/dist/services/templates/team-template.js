...
// exportTeam
// ---------------------------------------------------------------------------
export async function exportTeam(projectId) {
    const db = getDb();
    const agents = await db
        .select()
        .from(schema.agents)
        .where(eq(schema.agents.projectId, projectId));
    const agentExports = await Promise.all(agents.map(async (agent) => {
...
export async function importTeam(projectId, payload, opts = {}) {
    const db = getDb();
    const pool = getPool();
      ...
      `, [
            ...
            if (agentDef.skills.length > 0) {
                const skillRows = await db
                    ...
                    .from(schema.skills)
                    .where(and(eq(schema.skills.projectId, projectId), inArray(schema.skills.key, agentDef.skills)));
          ...
          `, [agentId, skill.id]);
            ...
            if (agentDef.tools.length > 0) {
                const toolRows = await db
                    ...
                    .from(schema.tools)
                    .where(and(eq(schema.tools.projectId, projectId), inArray(schema.tools.key, agentDef.tools)));
          ...
          `, [agentId, tool.id]);
            ...
            if (agentDef.mcpServers.length > 0) {
                const mcpRows = await db
                    ...
                    .where(and(eq(schema.mcpServers.projectId, projectId), inArray(schema.mcpServers.name, agentDef.mcpServers)));
...
// instantiateTemplate
// ---------------------------------------------------------------------------
export async function instantiateTemplate(projectId, templateId, opts = {}) {
    const db = getDb();
    const [tmpl] = await db
        .select()
    ...
    if (tmpl.kind !== 'team')
        throw new Error(`Template ${templateId} is not a team template`);
    return importTeam(projectId, tmpl.payload, opts);
}
//# sourceMappingURL=team-template.js.map