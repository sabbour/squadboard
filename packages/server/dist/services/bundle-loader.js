...
async function readBodyPath(bodyPath, bundleDir) {
...
// Section: project
// ---------------------------------------------------------------------------
async function applyProject(bundle, opts, result) {
    const db = getDb();
    if (opts.projectId) {
...
// ---------------------------------------------------------------------------
async function applyKanban(bundle, projectId, opts, result) {
    if (!bundle.kanban)
        return;
    const pool = getPool();
    // Check existing columns.
    const { rows: existing } = await pool.query(`SELECT column_id FROM column_meta WHERE project_id = $1`, [projectId]);
    const existingSlugs = new Set(existing.map((r) => r.column_id));
    if (existingSlugs.size > 0 && !opts.overwriteExisting) {
        result.skipped.push(`kanban: ${existingSlugs.size} column(s) already exist — skipped (use overwriteExisting to replace)`);
        return;
    }
    ...
    for (let i = 0; i < sorted.length; i++) {
        ...
        await pool.query(`INSERT INTO column_meta
         (project_id, column_id, label, color, position, semantic, is_default)
       VALUES ($1, $2, $3, $4, $5, $6, $7)
       ON CONFLICT (project_id, column_id) DO NOTHING`, [projectId, col.slug, col.label, '#6B6B6B', i, 'custom', isDefault]);
...
// ---------------------------------------------------------------------------
async function applyTeam(bundle, projectId, opts, result) {
    if (!bundle.team || bundle.team.length === 0)
        return;
    const db = getDb();
...
async function insertWorkflowWithVersion(db, params) {
 ...
 */
function normalizeCeremonyTrigger(trigger) {
...
async function applyCeremonies(bundle, projectId, opts, result) {
    if (!bundle.ceremonies || bundle.ceremonies.length === 0)
        return;
    const db = getDb();
    const existing = await db
...
// ---------------------------------------------------------------------------
async function applyWorkflows(bundle, projectId, opts, result) {
...
// Section: skills
// ---------------------------------------------------------------------------
async function applySkills(bundle, projectId, opts, result) {
...
// Section: tools
// ---------------------------------------------------------------------------
async function applyTools(bundle, projectId, opts, result) {
...
// Section: MCP servers
// ---------------------------------------------------------------------------
async function applyMcpServers(bundle, projectId, opts, result) {
...
// ---------------------------------------------------------------------------
async function applyRouting(bundle, projectId, opts, result) {
    if (!bundle.routing || bundle.routing.length === 0)
        return;
    const db = getDb();
    const existing = await db
...
export async function applyBundle(bundle, opts = {}) {
    ...
    if (bundle.manifest.schemaVersion > 1) {
        result.warnings.push(`Bundle schemaVersion=${bundle.manifest.schemaVersion} is newer than supported (1). Unknown sections will be ignored.`);
    }
 ...
 */
export async function loadBundle(pathOrUrl) {
