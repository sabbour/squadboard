...
function inferMatchType(pattern) {
    ...
    if (pattern.startsWith('#') || pattern.toLowerCase().startsWith('label:'))
        return 'label';
    // backtick-wrapped labels (markdown inline code used in Issue Routing table)
    if (/^`[^`]+`$/.test(pattern.trim()))
        return 'label';
    if (p === '*' || p.toLowerCase() === 'all' || p.toLowerCase() === 'catchall')
        return 'catchall';
    return 'keyword';
}
/**
 ...
 */
export async function parseRoutingFile(routingMdPath) {
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        // Data row
        if (cells.length <= Math.max(patternColIdx, agentColIdx))
            continue;
        ...
        if (agentRaw.includes(':') && agentRaw.split(' ').length > 2)
            continue;
        // Skip separator or header repetitions
        if (pattern.toLowerCase() === headers[patternColIdx])
            continue;
 ...
 */
export function matchRule(rule, issue) {
...
//# sourceMappingURL=routing-compiler.js.map