 ...
 */
function findSquadDirUpward(start) {
 ...
 */
export function locateRepoSquadDir() {
 ...
 */
export async function registerSelfIfNeeded() {
    if (!isEnabled()) {
 ...
 * tests + diagnostics.
 */
export async function registerSelfAtBoot() {
    try {
        const result = await registerSelfIfNeeded();
        if (result.registered) {
            console.log(`[self-register] inserted project "${SELF_REGISTER_NAME}" id=${result.projectId} path=${result.squadDir}`);
        }
        else if (result.reason === 'already_registered') {
            console.log(`[self-register] skipped — already registered as id=${result.projectId} path=${result.squadDir}`);
        }
            ...
            console.log('[self-register] skipped — disabled (NODE_ENV=production and SQUADBOARD_AUTO_REGISTER_SELF != true)');
        }
        return result;
    }
    catch (err) {
...
//# sourceMappingURL=self-register.js.map