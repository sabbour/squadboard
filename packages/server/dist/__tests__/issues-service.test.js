...
vi.mock('drizzle-orm', () => ({
    and: (...args) => ({ __and: args }),
    eq: (col, val) => ({ __eq: [col, val] }),
    sql: Object.assign((strings, ...vals) => ({ __sql: { strings, vals } }), { __brand: 'sql' }),
}));
// ---------------------------------------------------------------------------
...
describe('createIssue()', () => {
    ...
    it('sets completedAt when status=done and no explicit completedAt', async () => {
        ...
        const completedMs = valuesArg.completedAt.getTime();
...
//# sourceMappingURL=issues-service.test.js.map