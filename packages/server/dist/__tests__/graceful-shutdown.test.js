...
// ---------------------------------------------------------------------------
async function runStaleRunRecovery() {
    const { getPool } = await import('../db/index.js');
    const pool = getPool();
    const { rows } = await pool.query(`UPDATE issue_runs
        SET status       = 'failed',
...
// ---------------------------------------------------------------------------
async function runShutdownCallback() {
...
describe('graceful shutdown — clean-close path', () => {
    ...
    it('still calls closeDb even if CHECKPOINT throws', async () => {
        ...
        try {
            await mockPglite.exec('CHECKPOINT');
        }
        catch { /* swallowed */ }
        await mockCloseDb();
        expect(mockCloseDb).toHaveBeenCalledOnce();
