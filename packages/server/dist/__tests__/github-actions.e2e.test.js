...
const ghResponses = {};
function registerGhResponse(args, response) {
    const key = args.join(' ');
    ghResponses[key] = response;
}
function findGhResponse(args) {
    // Exact match first.
    const key = args.join(' ');
    if (ghResponses[key]) {
...
vi.mock('node:child_process', async (importOriginal) => {
    ...
    return {
        ...original,
        execFile: vi.fn((_file, args, _optsOrCb, cbMaybe) => {
...
beforeAll(async () => {
    ...
    eventBus.on('event', (e) => capturedEvents.push(e));
    // Seed test data via the pool (raw SQL is simplest for bootstrapping).
    const pool = dbContainer.pool;
    const projResult = await pool.query(`INSERT INTO projects (name, path, github_owner, github_repo, github_sync_enabled)
     VALUES ('Test Project', '.', 'acme', 'myrepo', true)
     RETURNING id`);
    projectId = projResult.rows[0].id;
    const issueResult = await pool.query(`INSERT INTO issues (project_id, title, github_issue_number)
...
describe('G5.1 — assign-to-copilot → PR detection → comment → merge', () => {
    it('G4.1 assign-to-copilot via gh issue assign creates issue_runs row', async () => {
        ...
        registerGhResponse(['issue', 'edit', '42', '--add-assignee', 'copilot', '--repo', 'acme/myrepo'], { stdout: '' });
        ...
        const pool = dbContainer.pool;
        const agentResult = await pool.query(`INSERT INTO agents (project_id, name, role, status, charter_path, agent_kind)
       VALUES ($1, '@copilot', 'copilot-coding-agent', 'active', '@copilot', 'copilot')
       RETURNING id`, [projectId]);
        const copilotAgentId = agentResult.rows[0].id;
        const runResult = await pool.query(`INSERT INTO issue_runs (issue_id, agent_id, status, workspace_strategy, external_ref)
       VALUES ($1, $2, 'pending', 'scratch', '{"owner":"acme","repo":"myrepo","issueNumber":42}')
       RETURNING id`, [issueId, copilotAgentId]);
        runId = runResult.rows[0].id;
        expect(runId).toBeTruthy();
        // Verify the row has external_ref persisted.
        const runRow = await pool.query(`SELECT external_ref, status FROM issue_runs WHERE id = $1`, [runId]);
    ...
    it('G4.2 watcher tick detects copilot draft PR and updates issue_runs', async () => {
        ...
        registerGhResponse(['pr', 'list', '--repo', 'acme/myrepo', '--author', 'copilot', '--state', 'open',
        ...
        const prDetectedEvents = [];
        const handler = (e) => {
            if (e.type === 'copilot.pr.detected')
                prDetectedEvents.push(e);
        };
        ...
        const pool = dbContainer.pool;
        const row = await pool.query(`SELECT pr_number, pr_url, pr_state FROM issue_runs WHERE id = $1`, [runId]);
        ...
        const payload = prDetectedEvents[0].payload;
    ...
    it('G2.3 commentOnIssue posts a comment and emits git.comment.posted', async () => {
        ...
        registerGhResponse(['issue', 'comment', '42', '--body-file', '-'], { stdout: 'https://github.com/acme/myrepo/issues/42#issuecomment-999\n' });
    ...
    it('G2.5 mergePr with passing CI marks prState=merged and emits git.pr.merged', async () => {
        ...
        const pool = dbContainer.pool;
        await pool.query(`UPDATE issue_runs SET pr_number = 99, pr_url = 'https://github.com/acme/myrepo/pull/99', workspace_path = '/fake/ws' WHERE id = $1`, [runId]);
        // Stub: pr checks passes (no output, exit 0).
        registerGhResponse(['pr', 'checks', '99', '--required'], { stdout: '' });
        // Stub: pr merge succeeds.
        registerGhResponse(['pr', 'merge', '99', '--squash', '--delete-branch'], { stdout: 'a'.repeat(40) + '\n' });
        ...
        const row = await pool.query(`SELECT pr_state, status FROM issue_runs WHERE id = $1`, [runId]);
    ...
    it('G4.3 label rule idempotency: duplicate dispatch blocked', async () => {
        const pool = dbContainer.pool;
        // Create a rule.
        const ruleResult = await pool.query(`INSERT INTO copilot_auto_assign_rules (project_id, label, enabled)
       VALUES ($1, 'needs-copilot', true)
       RETURNING id`, [projectId]);
        const ruleId = ruleResult.rows[0].id;
        // Record a dispatch for this (rule, issue) pair.
        await pool.query(`INSERT INTO copilot_auto_assign_dispatches (rule_id, issue_id)
       VALUES ($1, $2)`, [ruleId, issueId]);
        ...
        const result = await pool.query(`INSERT INTO copilot_auto_assign_dispatches (rule_id, issue_id)
       VALUES ($1, $2)
       ON CONFLICT DO NOTHING`, [ruleId, issueId]);
        expect(result.rowCount).toBe(0); // second dispatch blocked
    });
});
//# sourceMappingURL=github-actions.e2e.test.js.map