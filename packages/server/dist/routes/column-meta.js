...
const VALID_SEMANTICS = ['backlog', 'ready', 'in_progress', 'review', 'done', 'custom'];
...
async function seedDefaults(projectId) {
    ...
    for (const d of COLUMN_DEFAULTS) {
        await pool.query(`INSERT INTO column_meta (project_id, column_id, label, description, color, position, semantic, is_default)
       VALUES ($1, $2, $3, $4, $5, $6, $7, $8)
       ON CONFLICT (project_id, column_id) DO NOTHING`, [projectId, d.columnId, d.label, d.description, d.color, d.position, d.semantic, d.isDefault]);
    }
}
...
router.get('/', async (req, res) => {
    try {
        ...
        const { rows } = await pool.query(`SELECT id, column_id AS "columnId", label, description, color, position,
...
router.post('/', async (req, res) => {
    try {
        const { projectId } = req.params;
        const { columnId, label, description, color, position, semantic } = req.body;
        // Validate columnId
        ...
        const resolvedSemantic = (semantic === undefined ? 'custom' : semantic);
        ...
        try {
            ...
            if (typeof position === 'number') {
                ...
                await client.query(`UPDATE column_meta SET position = position + 1, updated_at = NOW()
           WHERE project_id = $1 AND position >= $2`, [projectId, targetPosition]);
            }
            else {
                const { rows: maxRows } = await client.query(`SELECT COALESCE(MAX(position), -1) AS max FROM column_meta WHERE project_id = $1`, [projectId]);
                targetPosition = (maxRows[0]?.max ?? -1) + 1;
            }
            const { rows } = await client.query(`INSERT INTO column_meta (project_id, column_id, label, description, color, position, semantic, is_default)
         VALUES ($1, $2, $3, $4, $5, $6, $7, false)
         RETURNING id, column_id AS "columnId", label, description, color, position,
                   semantic, is_default AS "isDefault"`, [projectId, columnId, label.trim(), description ?? null, color, targetPosition, resolvedSemantic]);
            ...
            if (err instanceof Error &&
                'code' in err &&
                err.code === '23505') {
                res.status(409).json({ ok: false, error: `Column '${columnId}' already exists for this project` });
                return;
            }
            throw err;
        }
    ...
    try {
        ...
        try {
            ...
            for (let i = 0; i < order.length; i++) {
                await client.query(`UPDATE column_meta SET position = $1, updated_at = NOW()
           WHERE project_id = $2 AND column_id = $3`, [i, projectId, order[i]]);
            }
            const { rows } = await client.query(`SELECT id, column_id AS "columnId", label, description, color, position,
                semantic, is_default AS "isDefault"
         FROM column_meta
         WHERE project_id = $1
         ORDER BY position ASC`, [projectId]);
...
router.patch('/:columnId', async (req, res) => {
    try {
        ...
        const { label, description, color, semantic, isDefault } = req.body;
        // Validate inputs
        if (label !== undefined) {
            if (typeof label !== 'string' || label.trim().length === 0 || label.length > 80) {
                res.status(400).json({ ok: false, error: 'label must be a non-empty string of max 80 chars' });
        ...
        if (semantic !== undefined) {
            if (!VALID_SEMANTICS.includes(semantic)) {
                res.status(400).json({ ok: false, error: `semantic must be one of: ${VALID_SEMANTICS.join(', ')}` });
                return;
            }
        }
        ...
        const params = [projectId, columnId];
        let idx = 3;
        if (label !== undefined) {
            sets.push(`label = $${idx++}`);
        ...
        if (semantic !== undefined) {
            sets.push(`semantic = $${idx++}`);
            params.push(semantic);
        }
        ...
        if (isDefault === true) {
            ...
            try {
                await client.query('BEGIN');
                await client.query(`UPDATE column_meta SET is_default = false, updated_at = NOW() WHERE project_id = $1`, [projectId]);
                sets.push(`is_default = true`);
                const { rows } = await client.query(`UPDATE column_meta
           SET ${sets.join(', ')}
           WHERE project_id = $1 AND column_id = $2
           RETURNING id, column_id AS "columnId", label, description, color, position,
                     semantic, is_default AS "isDefault"`, params);
                await client.query('COMMIT');
                if (rows.length === 0) {
        ...
        const { rows } = await pool.query(`UPDATE column_meta
       SET ${sets.join(', ')}
       WHERE project_id = $1 AND column_id = $2
       RETURNING id, column_id AS "columnId", label, description, color, position,
                 semantic, is_default AS "isDefault"`, params);
...
router.delete('/:columnId', async (req, res) => {
    try {
        ...
        try {
            ...
            const { rows: colRows } = await client.query(`SELECT id FROM column_meta WHERE project_id = $1 AND column_id = $2`, [projectId, columnId]);
            if (colRows.length === 0) {
                await client.query('ROLLBACK');
                res.status(404).json({ ok: false, error: 'Column not found' });
                return;
            ...
            const { rows: countRows } = await client.query(`SELECT COUNT(*) AS count FROM column_meta WHERE project_id = $1`, [projectId]);
            ...
            const { rows: issueCountRows } = await client.query(`SELECT COUNT(*) AS count FROM issues WHERE project_id = $1 AND status = $2 AND archived = 0`, [projectId, columnId]);
            ...
            if (issueCount > 0) {
                ...
                const { rows: targetRows } = await client.query(`SELECT id FROM column_meta WHERE project_id = $1 AND column_id = $2`, [projectId, reassignTo]);
                ...
                await client.query(`UPDATE issues SET status = $1, updated_at = NOW()
           WHERE project_id = $2 AND status = $3 AND archived = 0`, [reassignTo, projectId, columnId]);
            }
            // Delete the column.
            await client.query(`DELETE FROM column_meta WHERE project_id = $1 AND column_id = $2`, [projectId, columnId]);
            // If this was the default column, assign default to the lowest-position remaining column.
            await client.query(`UPDATE column_meta SET is_default = true, updated_at = NOW()
         WHERE id = (
