...
projectDeliverablesRouter.get('/', async (req, res) => {
    try {
        ...
        const issueId = req.query.issueId?.trim();
        const db = getDb();
        if (issueId) {
            // Validate ownership.
...
projectDeliverablesRouter.post('/:id/review', async (req, res) => {
    try {
        ...
        if (body.verb === 'approve')
            newStatus = 'approved';
        if (body.verb === 'request_changes')
            newStatus = 'changes_requested';
        if (newStatus)
            await deliverablesService.setDeliverableStatus(id, newStatus);
        // Emit + system comment.
        ...
        let spawnedRunId = null;
        if (body.verb === 'request_changes' && body.spawnRevision) {
            try {
                spawnedRunId = await spawnRevisionRun(projectId, deliverable, body.spawnRevision.agentId ?? null, body.body ?? null);
            }
            catch (err) {
...
projectDeliverablesRouter.post('/:id/peer-review', async (req, res) => {
    try {
        ...
        if (!agentId) {
            ...
            const decision = await resolveRouteFull(projectId, { title: issue.title, labels: [], body: issue.body ?? '' }, issue.id);
        ...
        const [agent] = await db
            ...
            .limit(1);
        if (!agent)
            return res.status(404).json({ error: 'Agent not found in this project' });
        // Build the input context: a structured slice of the deliverable payload.
        const inputContext = `Peer review request for deliverable: ${deliverable.title}\n\n` +
...
// ---------------------------------------------------------------------------
async function spawnRevisionRun(projectId, deliverable, agentId, changeRequestBody) {
    const db = getDb();
    ...
    if (!chosenAgentId) {
        ...
        if (decision.agentId) {
            ...
            const agentName = decision.agentName ?? '';
            if (!agentName)
                throw new Error('Router returned no agent for revision');
            const [resolved] = await db
                .select()
                .from(schema.agents)
                .where(and(eq(schema.agents.projectId, projectId), eq(schema.agents.name, agentName)))
                .limit(1);
            if (!resolved)
                throw new Error(`Router agent '${agentName}' not found`);
    ...
    const inputContext = `Revision requested on deliverable: ${deliverable.title}\n\n` +
    ...
    const [created] = await db
        ...
        .returning();
    if (!created)
        throw new Error('Failed to insert revision run');
    return created.id;
}
