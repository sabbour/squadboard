...
import { startLiveSession, sendPromptToSession, } from '../sdk/squad-stream.js';
...
commentsMentionRouter.post('/:id/mention', async (req, res) => {
    ...
    const agentRows = await db
        .select()
        .from(schema.agents)
        .where(and(eq(schema.agents.projectId, projectId), inArray(schema.agents.name, mentions)));
    ...
    const activeSessions = await db
        ...
        .where(and(eq(schema.liveSessions.projectId, projectId), eq(schema.liveSessions.status, 'active')));
    const sessionsByAgentId = new Map();
    for (const s of activeSessions) {
        if (s.agentId)
            sessionsByAgentId.set(s.agentId, s);
    }
    const results = [];
    for (const mention of mentions) {
        ...
        const preamble = `You were mentioned on Squadboard issue "${issueTitle}". ` +
...
//# sourceMappingURL=comments-mention.js.map