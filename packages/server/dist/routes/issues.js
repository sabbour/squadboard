        ...
        try {
            ...
            if (match) {
                ...
                const [agent] = await getDb()
                    .select()
                    .from(schema.agents)
                    .where(and(eq(schema.agents.projectId, projectId), eq(schema.agents.name, match.agentName)));
                if (agent) {
...
router.post('/bulk', async (req, res) => {
    try {
        const { projectId } = req.params;
        const { action, issueIds, status, labelIds } = req.body;
...
router.post('/formulate', async (req, res) => {
    try {
        ...
        if (status >= 500)
            console.error('[issues/formulate] unhandled:', err);
    ...
    try {
        ...
        if (version !== undefined) {
            ...
            const [updated] = await db
                .update(issues)
                .set(patch)
                .where(and(eq(issues.id, id), eq(issues.projectId, projectId), eq(issues.version, version), eq(issues.archived, 0)))
                .returning();
            if (!updated) {
...
router.post('/:issueId/attachments', upload.single('file'), async (req, res) => {
    try {
...
router.get('/:issueId/attachments/:attachmentId', async (req, res) => {
    try {
        const { projectId, issueId, attachmentId } = req.params;
