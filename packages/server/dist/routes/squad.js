...
router.post('/register', async (req, res) => {
    const { path: squadPath, name } = req.body;
    if (!squadPath) {
...
// Scaffold helpers
// ---------------------------------------------------------------------------
async function scaffoldSquad(squadPath, projectName) {
    await fs.mkdir(squadPath, { recursive: true });
    await fs.writeFile(path.join(squadPath, 'team.md'), `# ${projectName}\n\n## Members\n\n| Name | Role | Status |\n|------|------|--------|\n`, 'utf-8');
...
async function registerProject(squadPath, projectName) {
    const db = getDb();
    const existing = await db
        .select()
        .from(schema.projects)
...
router.post('/init', async (req, res) => {
    const { path: dirPath, projectName } = req.body;
    if (!dirPath) {
        res.status(400).json({ ok: false, error: 'Body field "path" is required' });
        return;
    }
