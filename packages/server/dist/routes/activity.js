...
router.get('/now', async (_req, res) => {
    try {
        ...
        const { rows: sessionRows } = await pool.query(`SELECT
         ls.id,
       ...
       LIMIT 50`, ACTIVE_SESSION_STATUSES);
        // ── 2. Issue Runs ─────────────────────────────────────────────────────
        const runPlaceholders = ACTIVE_RUN_STATUSES.map((_, i) => `$${i + 1}`).join(', ');
        const { rows: runRows } = await pool.query(`SELECT
         ir.id,
       ...
       LIMIT 50`, ACTIVE_RUN_STATUSES);
        // ── 3. Workflow Runs ──────────────────────────────────────────────────
        const wfPlaceholders = ACTIVE_RUN_STATUSES.map((_, i) => `$${i + 1}`).join(', ');
        const { rows: workflowRows } = await pool.query(`SELECT
         wr.id,
         i.project_id AS "projectId",
         p.name       AS "projectName",
