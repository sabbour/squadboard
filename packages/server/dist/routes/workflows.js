...
issueWorkflowRouter.post('/', async (req, res) => {
    try {
        const { issueId } = req.params;
        const { workflowVersionId } = req.body;
...
issueWorkflowRouter.delete('/', async (req, res) => {
    try {
        const { issueId } = req.params;
        const db = getDb();
        await db
...
issueWorkflowRouter.post('/start', async (req, res) => {
    try {
        const { issueId } = req.params;
        const db = getDb();
 ...
 */
workflowRunsRouter.get('/:id/reviews', async (req, res) => {
    try {
        const { id } = req.params;
        const db = getDb();
 ...
 */
stepRunsRouter.post('/:id/review', async (req, res) => {
    try {
        const { id } = req.params;
        const { verb, body: reviewBody, suggestions } = req.body;
        ...
        if (verb === 'approve') {
            ...
            if (priorIndex >= 0) {
                const [priorStep] = await db
                    .select()
                    .from(schema.stepRuns)
                    .where(and(eq(schema.stepRuns.workflowRunId, wfRun.id), eq(schema.stepRuns.stepIndex, priorIndex)))
                    .limit(1);
                if (priorStep) {
