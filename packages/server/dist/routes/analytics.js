...
router.get('/overview', async (req, res) => {
    ...
    `);
        const doneRow = (doneWeekRows.rows[0] ?? { this_week: 0, last_week: 0 });
    ...
    `);
        const activeAgents = (agentRows.rows[0] ?? { count: 0 }).count;
    ...
    `);
        const workflowsActive = (wfRows.rows[0] ?? { count: 0 }).count;
    ...
    `);
        const openReviews = (reviewRows.rows[0] ?? { count: 0 }).count;
        res.json({
            issuesByStatus,
