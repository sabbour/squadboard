 ...
 */
export async function loadRoutingRules(projectId, squadPath) {
    ...
    if (parsed.length > 0) {
        await db.insert(routingRules).values(parsed.map((r) => ({
            projectId,
            priority: r.priority,
            pattern: r.pattern,
 ...
 */
function extractCharterKeywords(charterContent) {
 ...
 */
export async function refreshAgentKeywords(projectId) {
    const db = getDb();
 ...
 */
export async function resolveRoute(projectId, issue) {
    const db = getDb();
    const { routingRules } = schema;
 ...
 */
export async function resolveRouteTier2(projectId, issue) {
    const db = getDb();
    const { agents, agentKeywords } = schema;
    ...
    for (const agent of activeAgents) {
        ...
        if (!cached) {
            ...
            if (dbRow) {
                cached = {
                    ...
                    focusAreas: JSON.parse(dbRow.focusAreas),
                };
        ...
        if (score > 0 && (!best || score > best.score)) {
            ...
            if (matchedKeywords.length)
                reasonParts.push(`keywords: [${matchedKeywords.slice(0, 5).join(', ')}]`);
            if (matchedLabels.length)
                reasonParts.push(`labels: [${matchedLabels.join(', ')}]`);
            best = {
                agentId: agent.id,
 ...
 */
export async function resolveRouteTier3(projectId, issue, issueId) {
    const db = getDb();
    ...
    const agentList = activeAgents
        .map((a) => `- ${a.name}: ${a.role}`)
        .join('\n');
    const routingTask = `You are a routing agent. Given the issue below, select the best agent from the list.\n` +
    ...
    const resolvedAgent = activeAgents.find((a) => a.name.toLowerCase() === assignee.toLowerCase()) ?? activeAgents[0];
    await db
        .update(issueRuns)
        .set({
        status: 'completed',
 ...
 */
export async function resolveRouteFull(projectId, issue, issueId) {
 ...
 */
export async function createRoutedRun(issueId, agentId, resolvedByRule, routingTier, routingScore, routingReasoning) {
    const db = getDb();
