 ...
 */
export async function createPeerReviewRuns(workflowRunId, stepRunId, issueId, reviewerAgentIds, priorOutput) {
    const db = getDb();
    ...
    const inserted = await db
        .insert(issueRuns)
        .values(reviewerAgentIds.map((agentId) => ({
        issueId,
        agentId,
        kind: 'peer_review',
        status: 'pending',
    ...
    console.log(`[peer-reviewer] created ${ids.length} peer_review run(s) for workflow_run ${workflowRunId}, step_run ${stepRunId}: [${ids.join(', ')}]`);
    return ids;
}
// ---------------------------------------------------------------------------
 ...
 */
export async function collectReviewDecisions(stepRunId) {
    const db = getDb();
    const { issueRuns } = schema;
    const reviewRuns = await db
        .select()
        .from(issueRuns)
        .where(and(eq(issueRuns.stepRunId, stepRunId), eq(issueRuns.kind, 'peer_review')));
    if (reviewRuns.length === 0) {
        return { allComplete: false, decisions: [] };
    }
    const allComplete = reviewRuns.every((r) => r.status === 'completed' || r.status === 'failed' || r.status === 'cancelled');
    const decisions = [];
 ...
 */
export function shouldBlock(decisions, policy, quorum) {
 ...
 */
export function isQuorumMet(decisions, quorum) {
    if (!quorum)
        return true; // no quorum configured — any approvals advance
    const approveCount = decisions.filter((d) => d.decision === 'approve').length;
 ...
 */
export async function injectReviewerFeedback(stepRunId, workflowRunId, decisions) {
    const db = getDb();
    ...
    if (blockingDecisions.length > 0) {
        await db.insert(reviewEvents).values(blockingDecisions.map((d) => ({
            workflowRunId,
            stepRunId,
    ...
    console.log(`[peer-reviewer] injected feedback from ${blockingDecisions.length} reviewer(s) ` +
 ...
 */
export async function recordApproval(stepRunId, workflowRunId, decisions) {
    const db = getDb();
    const { stepRuns, reviewEvents } = schema;
    ...
    if (approvals.length > 0) {
        await db.insert(reviewEvents).values(approvals.map((d) => ({
            workflowRunId,
            stepRunId,
            issueRunId: d.issueRunId,
            reviewerAgentId: d.agentId,
    ...
    console.log(`[peer-reviewer] recorded ${approvals.length} approval(s) for step_run ${stepRunId}`);
}
// ---------------------------------------------------------------------------
// Helpers
// ---------------------------------------------------------------------------
 ...
 */
function buildReviewContext(priorOutput) {
    return [
        '## Work Under Review',
        '',
 ...
 */
function safeParseReviewOutput(output) {
...
//# sourceMappingURL=peer-reviewer.js.map