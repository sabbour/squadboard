 ...
 */
export async function spawnFanOutChildren(opts) {
    ...
    console.log(`[fan-out-adapter] parallel spawn complete: ${successCount}/${opts.children.length} ` +
...
// Internals
// ---------------------------------------------------------------------------
async function spawnChunk(children) {
    ...
    const configs = tagged.map(({ child, taggedName }) => {
        const config = {
        ...
        if (child.context)
            config.context = child.context;
        if (child.modelOverride)
            config.modelOverride = child.modelOverride;
        return config;
    });
    ...
    for (const sdkResult of sdkResults) {
        ...
        if (!child) {
            console.warn(`[fan-out-adapter] unknown taggedName from SDK: ${sdkResult.agentName} — skipping persist`);
            continue;
        }
        ...
        if (sdkResult.status === 'success' && sdkResult.sessionId) {
            try {
                ...
                const msg = err instanceof Error ? err.message : String(err);
                console.error(`[fan-out-adapter] failed to persist session_id for step_run ${child.stepRunId}:`, msg);
                out.push({
            ...
            try {
                ...
                console.error(`[fan-out-adapter] failed to mark step_run ${child.stepRunId} failed:`, err);
            }
            out.push({
                sessionId: sdkResult.sessionId,
                status: 'failed',
...
// ---------------------------------------------------------------------------
function buildDependencies(childByTag) {
    return {
        compileCharter: async (taggedName) => {
            const child = childByTag.get(taggedName);
            if (!child) {
                throw new Error(`compileCharter: unknown taggedName ${taggedName}`);
            }
            const content = await readFile(child.charterPath, 'utf8').catch(() => `(charter not found at: ${child.charterPath})`);
            return {
        ...
        createSession: async (config) => {
            ...
            const child = childByTag.get(taggedName);
            if (!child) {
                throw new Error(`createSession: cannot resolve child from clientName=${config.clientName}`);
            }
...
function makeMinimalSessionPool() {
...
function makeMinimalEventBus(childByTag) {
    return {
        emit: async (event) => {
            ...
            try {
                if (event.type === 'session.created') {
                    eventBus.emitSessionEvent('session.started', child?.workflowRunId ?? 'global', enriched);
                }
                else if (event.type === 'session.error') {
                    eventBus.emitSessionEvent('session.error', child?.workflowRunId ?? 'global', enriched);
                }
            }
            catch (err) {
...
// ---------------------------------------------------------------------------
async function markStepRunRunning(stepRunId, sessionId) {
    const db = getDb();
...
async function markStepRunFailed(stepRunId, errorMessage) {
    ...
    await pool.query(`UPDATE step_runs
        SET status      = 'failed',
            output      = $1,
            updated_at  = NOW()
      WHERE id = $2`, [tagged, stepRunId]);
}
//# sourceMappingURL=fan-out-adapter.js.map