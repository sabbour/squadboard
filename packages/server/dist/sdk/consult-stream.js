 ...
 */
export async function startConsultSession(input) {
    ...
    if (input.mode === 'agent' && resolvedAgentId) {
        ...
        if (row) {
            ...
            if (row.status !== 'active') {
                throw Object.assign(new Error(`Agent "${row.name}" is ${row.status} — re-enable it before starting a consult.`), { status: 422 });
            }
 ...
 */
export async function sendConsultMessage(sessionId, userMessage) {
 ...
 */
export async function runConsultTurn(sessionId, userMessage) {
    await runner(sessionId, userMessage);
}
let runner = defaultRunner;
export function setConsultTurnRunner(fn) {
    runner = fn;
}
async function defaultRunner(sessionId, userMessage) {
 ...
 */
async function streamAgentTurn(sessionId, userMessage) {
    const session = await consultService.getConsultSession(sessionId);
    if (!session)
        return;
...
async function fallbackStubReply(sessionId, errorMsg) {
...
async function openSdkConsult(session) {
    const db = getDb();
    // Resolve the model.
    let agentModel = null;
...
function attachListeners(running) {
    ...
    running.session.on('assistant.reasoning_delta', (e) => {
        ...
        if (!delta)
            return;
        if (running.pendingAssistant)
            running.pendingAssistant.reasoning += delta;
 ...
 */
export async function endRunningConsult(consultId, reason = 'cancelled') {
...
export async function shutdownAllConsults() {
 ...
 */
function buildProposeTools(consultId) {
    const handle = async (kind, payload, callId, description) => {
    ...
    return [
        {
            name: 'propose_issue',
            description: 'Propose a new issue / card on the project board. Renders as a card the user can Accept, Edit, or Discard. Do not call unless the user clearly wants to capture a unit of work.',
            parameters: {
                type: 'object',
                properties: {
                    ...
                    projectId: {
                        type: 'string',
                        description: 'Project UUID. Optional in project-scoped consults (defaults to the consult\'s project). Required in cross-project consults.',
                    },
                    columnSlug: {
                        type: 'string',
        ...
        },
        {
            ...
            description: 'Propose a new narrative ceremony (Markdown). The user will Accept, Edit, or Discard before it lands as a draft for the Ceremonies Review page.',
            parameters: {
                type: 'object',
                properties: {
        ...
        },
        {
            ...
            description: 'Propose a captured idea / draft for the Inbox — not yet a structured issue. The user will refine and decide what project to file it under.',
            parameters: {
                type: 'object',
                properties: {
        ...
        },
        {
            ...
            description: 'Propose recording a decision in this project\'s .squad/decisions/inbox/ as a Markdown note (one decision per file). Use for choices the user explicitly wants captured for posterity.',
            parameters: {
                type: 'object',
                properties: {
        ...
        },
        {
            ...
            description: 'Propose assigning a specific agent to an existing issue, which on Accept creates a routed run.',
            parameters: {
                type: 'object',
                properties: {
            ...
            handler: async (args, inv) => {
                const a = (args ?? {});
                return handle('assign_agent_to_issue', a, inv.toolCallId, `assign ${String(a.agentName ?? '')} → ${String(a.issueId ?? '')}`);
            },
        },
    ];
}
export async function acceptProposal(input) {
    ...
    const effectivePayload = input.editedPayload && Object.keys(input.editedPayload).length > 0
        ? { ...proposal.payload, ...input.editedPayload }
        : proposal.payload;
    const isEdited = Boolean(input.editedPayload && Object.keys(input.editedPayload).length > 0);
    let artifact = null;
    try {
        artifact = await dispatchProposal(proposal.kind, effectivePayload, session);
    }
    catch (err) {
        const msg = err instanceof Error ? err.message : String(err);
        const updated = await consultService.updateProposal(input.proposalId, {
            status: 'pending',
            errorMessage: msg,
            ...(isEdited ? { editedPayload: input.editedPayload } : {}),
        });
...
export async function discardProposal(proposalId) {
...
async function dispatchProposal(kind, payload, session) {
    switch (kind) {
        case 'issue':
...
async function acceptProposeIssue(payload, session) {
    ...
    if (!title)
        throw new Error('propose_issue requires a title');
...
async function acceptProposeInboxItem(payload, session) {
    const summary = typeof payload.summary === 'string' ? payload.summary.trim() : '';
    if (!summary)
        throw new Error('propose_inbox_item requires a summary');
    const suggestedProjectId = (typeof payload.suggestedProjectId === 'string' && payload.suggestedProjectId) ||
        session.projectId ||
        null;
...
async function acceptProposeCeremony(payload, session) {
    const projectId = (typeof payload.projectId === 'string' && payload.projectId) ||
        session.projectId ||
        null;
    if (!projectId) {
    ...
    const [narrative] = await db
        ...
        .returning();
    if (!narrative)
        throw new Error('Failed to insert narrative ceremony');
...
async function acceptProposeCaptureDecision(payload, session) {
    const key = typeof payload.key === 'string' ? payload.key.trim() : '';
    const value = typeof payload.value === 'string' ? payload.value.trim() : '';
    if (!key || !value)
        throw new Error('propose_capture_to_decision requires `key` and `value`');
    if (!session.projectId) {
    ...
    const [proj] = await db
        ...
        .limit(1);
    if (!proj?.path)
        throw new Error('Project squad path not found');
...
async function acceptProposeAssignAgent(payload, session) {
    ...
    if (!issueId)
        throw new Error('propose_assign_agent_to_issue requires `issueId`');
    if (!agentName)
        throw new Error('propose_assign_agent_to_issue requires `agentName`');
    if (!session.projectId) {
    ...
    if (!found)
        throw new Error(`Agent '${agentName}' not found in this project`);
    void agent;
 ...
 */
export async function promoteConsult(input) {
    ...
    if (input.kind === 'issue') {
        if (!projectId)
            throw Object.assign(new Error('issue promotion requires a projectId'), { status: 400 });
        const issuesService = await import('../services/issues.js');
        const body = summary
            ? `${finalBody}\n\n---\n\n<details><summary>Original consult transcript</summary>\n\n${transcriptMd}\n\n</details>`
            : transcriptMd;
        const created = await issuesService.createIssue(projectId, {
            title: finalTitle,
            body,
            status: input.columnSlug ?? 'backlog',
        });
        return {
            kind: 'issue',
            summarised,
    ...
    // ceremony
    if (!projectId)
        throw Object.assign(new Error('ceremony promotion requires a projectId'), { status: 400 });
    const db = getDb();
    const slug = slugify(summary?.title ?? detail.name ?? 'consult-ceremony');
    const ceremonySummary = summary;
...
function renderTranscript(messages, title) {
    const lines = [`# ${title}`, ''];
...
//# sourceMappingURL=consult-stream.js.map