...
async function handleCreateIssue(args) {
    const db = getDb();
    const { projectId, title, body = '', idempotencyKey, } = args;
        ...
        try {
            ...
            switch (name) {
                case 'squadboard_list_issues':
                    result = await handleListIssues(toolArgs);
                    break;
                case 'squadboard_create_issue':
                    result = await handleCreateIssue(toolArgs);
                    break;
                ...
                case 'squadboard_list_agents':
                    result = await handleListAgents(toolArgs);
                    break;
