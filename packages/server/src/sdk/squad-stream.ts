...
export interface StartSessionResult {
  sessionId: string;          // Squadboard live_session id (UUID)
...
function summariseTitle(prompt: string): string {
...
class RunningLiveSession {
  ...
  private readonly client: SquadClientLike;
  private readonly session: SquadSessionLike;
  ...
  private async onAssistantMessage(event: unknown): Promise<void> {
  ...
  private async onDelta(event: unknown): Promise<void> {
    const delta = pickString(event, 'delta', 'content', 'text') ?? '';
    if (!delta) return;
    await this.publish('session.delta', { delta });
  }

  private async onReasoning(event: unknown): Promise<void> {
    const delta = pickString(event, 'delta', 'content', 'text') ?? '';
    if (!delta) return;
    await this.publish('session.delta', { delta, kind: 'reasoning' });
  }

  private async onUsage(event: unknown): Promise<void> {
...
const runningSessions = new Map<string, RunningLiveSession>();
 ...
 */
export async function startLiveSession(input: StartSessionInput): Promise<StartSessionResult> {
  const db = getDb();
  ...
  if (!created) throw new Error('Failed to create live session row');
...
export async function sendPromptToSession(sessionId: string, prompt: string): Promise<void> {
 ...
 */
export async function interruptLiveSession(sessionId: string): Promise<boolean> {
  const running = runningSessions.get(sessionId);
  if (!running) {
    throw new Error('Session not running (it may have completed or the server restarted)');
  }
...
export function getRunningLiveSession(sessionId: string): { id: string; projectId: string } | null {
  const running = runningSessions.get(sessionId);
