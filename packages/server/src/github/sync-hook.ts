...
import { eventBus, type BusEvent } from '../realtime/event-bus.js';
...
async function getGhConfig(projectId: string): Promise<GhConfig | null> {
  const db = getDb();
...
async function handleIssueEvent(event: BusEvent): Promise<void> {
  const { type, projectId, payload } = event;
  const issue = payload as LocalIssue & { id: string };
  ...
  sync.pushIssue(issue).catch((err: unknown) => {
    const msg = err instanceof Error ? err.message : String(err);
    logError({
      projectId,
      direction: 'push',
...
// The event bus doesn't have comment events by default — we'll listen for a
// custom 'comment.created' event emitted from the issues route when a comment
// is posted. The payload must include { comment, githubIssueNumber }.
...
async function handleCommentEvent(event: BusEvent): Promise<void> {
  const { projectId, payload } = event;
  const { comment, githubIssueNumber } = payload as CommentEventPayload;
 ...
 */
export async function ingestWebhookIssue(
  projectId: string,
  action: string,
  ghIssue: {
    number: number;
 ...
 */
export function initGitHubSyncHooks(): void {
  ...
  eventBus.on('event', (event: BusEvent) => {
    ...
    if (type === 'issue.created' || type === 'issue.updated' || type === 'issue.deleted') {
      handleIssueEvent(event).catch((err: unknown) => {
    ...
    if ((type as string) === 'comment.created') {
      handleCommentEvent(event).catch((err: unknown) => {
