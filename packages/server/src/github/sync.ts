...
import { GitHubClient, GitHubSyncError, type GitHubIssue } from './client.js';
...
const activeSyncLoops = new Map<string, ReturnType<typeof setInterval>>();
...
export class GitHubSync {
   ...
   */
  async pushIssue(issue: LocalIssue): Promise<void> {
    const db = getDb();
   ...
   */
  async pushComment(comment: LocalComment, githubIssueNumber: number): Promise<void> {
    const db = getDb();

    // Comments are immutable on GitHub (we don't edit them after posting)
    if (comment.githubCommentId != null) return;
    ...
    for (const ghIssue of ghIssues) {
      try {
        ...
        const [localIssue] = await db
          ...
          .where(
            and(
              eq(schema.issues.projectId, this.projectId),
        ...
        if (localIssue) {
          ...
          const winner = this.resolveConflict(localIssue as unknown as LocalIssue, ghIssue);
          if (winner === 'github') {
            await db
              .update(schema.issues)
              .set({
   ...
   */
  async pushAllIssues(): Promise<{ pushed: number; errors: number }> {
    const db = getDb();
    ...
    for (const issue of issues) {
      try {
        await this.pushIssue(issue as unknown as LocalIssue);
        pushed++;
      } catch {
        errors++;
      }
...
export function stopSyncLoop(projectId: string): void {
...
export function stopAllSyncLoops(): void {
