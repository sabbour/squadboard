...
async function handleList(args: string[]): Promise<SlashResult> {
  const db = getDb();
...
async function handleCreate(args: string[]): Promise<SlashResult> {
  const projectId = flagValue(args, '--project');
...
async function handleRun(args: string[]): Promise<SlashResult> {
...
async function handleStatus(args: string[]): Promise<SlashResult> {
  const [runId] = args;
...
async function handleAgents(args: string[]): Promise<SlashResult> {
  const projectId = flagValue(args, '--project');
  ...
  const lines = rows.map(
    (a) => `- **${a.name}** — ${a.role}${a.model ? ` (${a.model})` : ''} \`${a.id.slice(0, 8)}\``,
  );
  return {
    ok: true,
...
function helpResult(): SlashResult {
