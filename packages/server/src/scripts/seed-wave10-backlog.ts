...
const ITEMS: SeedItem[] = [
  ...
  },
  {
    ...
    prompt:
      'Fix bug: project tile hover resizes the tile. ProjectCard.tsx likely has a transform/scale on hover that grows the bounding box and reflows neighbours. Replace with elevation/shadow/border treatment that does not change layout dimensions.',
  },
  {
    id: 'B5',
  ...
  // ── Stream C — Fluent2 / UX polish ──────────────────────────────────────
  {
    ...
    prompt:
      'Polish: /consult/new page uses a narrow card centered in a wide page. Widen the card to the page-content width (canon: tokens.spacingHorizontalXXL). Re-balance the Agent | Model segmented control + Start-conversation button so they are not visually awkward.',
  },
  {
    id: 'C2',
    title: '[C2] AddProject "Connect existing" tab — horizontal-right dialog actions',
    prompt:
      'Polish: AddProject "Connect existing" tab has vertically stacked Connect/Close buttons. Standard Fluent2 dialog actions are horizontal-right. Move both buttons into <DialogActions> (consistent with Discover/Create tabs). Audit all three tabs for parity.',
  },
  {
    id: 'C3',
    title: '[C3] Now page — Fluent2 styling + aggregated view across projects',
    prompt:
      'Polish: Now page restyle to canon — replace inline status badge palette with Fluent\'s Badge component; switch table styling to DataGrid to match Costs/Templates. Verify useNowFeed API joins are project-agnostic and columns include a Project link. Fix any gaps.',
  },
  {
    id: 'C4',
    title: '[C4] Project selection dropdown wraps long names',
    prompt:
      'Polish: project selection dropdown wraps. Widen the popover (set minWidth on the menu surface) so single-line names like "Content Creation Workflow — Squad Edition" do not wrap. If width cannot accommodate the longest name, ellipsis + tooltip.',
  },
  {
    id: 'C5',
  ...
  },
  {
    ...
    prompt:
      'Polish: Costs table alignment — right-align numeric columns (Runs, Total Cost, Avg/Run, Tokens In/Out, Cost). Use DataGrid column align: end. Format USD with consistent precision; format tokens with thousands separators.',
  },
  {
    id: 'C7',
  ...
  },
  {
    ...
    prompt:
      'Polish: Diagnostics/Heartbeat scope is jarring — they exist at both global and per-project routes; clicking inside a project can drop you to global scope without warning. Recommend (a): keep both but make the page <PageHeader> clearly say "Global · across all projects" vs "Project: X" so the scope shift is visible.',
  },
  ...
  },
  {
    ...
    prompt:
      'Feature: tag each curated skill in the library modal with where it comes from — show "Source: built-in catalog" badge. Add Import skill action: file-pick a SKILL.md or skill.json; validate; persist to project skills. Same pattern as template import in Templates.tsx.',
  },
  {
    id: 'D3',
    title: '[D3] Import tool / Import MCP server — symmetric with D2',
    prompt:
      'Feature: Tools page — file-pick a tool.json import action. MCP page — file-pick an mcp-server.json import action. Add both to empty-state action set (see C5).',
  },
  {
    id: 'D4',
    title: '[D4] Ceremonies — show trigger source as labelled badge',
    prompt:
      'Feature: on Ceremony detail/list, when a ceremony is automatic (when: before|after with a condition), show the trigger source as a labelled badge (e.g. "Triggered from: routing.md → before any work batch matching X"). Inspect services/ceremony-translator.ts and the dispatcher for available metadata.',
  },
  {
    id: 'D5',
    title: '[D5] Ceremony create — drop duplicate "Prose" tab; keep "Formulate"',
    prompt:
      'Feature: CeremonyEditor.tsx exposes both Formulate-with-AI (top section) AND a Prose tab inside the Code/Visual/Prose tab strip. Keep Formulate-with-AI; remove the Prose tab. Migrate any Prose-only logic into the Formulate path so nothing is lost.',
  },
  {
    id: 'D6',
  ...
  },
  {
    ...
    prompt:
      'Feature: Flow page pivot — render agent runs as nodes with parent→child lineage edges. Each node: agent name + status (active/failed/done) + run id + step it is doing. Inspect pages/ProjectFlow.tsx + components/flow/. Reuse the runs data already on board; lineage edge is parentRunId → childRunId from issue_runs.',
  },
  ...
  // ── Stream E — Verification + close-out ─────────────────────────────────
  {
    ...
    prompt:
      'Verification: pnpm -r build clean; targeted Vitest/RTL tests on changed components (B4 hover, B5 reconnect, C2/C5/C6/C7 visual snapshots if framework supports); e2e: B1 template create, B2 Capture removal, A6 dogfood loop, D6 cost recording, D8 Flow agent-centric.',
  },
  {
    id: 'E2',
    title: '[E2] Docs refresh — MCP README, root README, mcp-config.json example',
    prompt:
      'Verification: update packages/server/src/mcp/README.md for the new dogfood flow (A2/A3) and the new pricing model (D6). Update root README.md Quickstart with the dogfood section. Update .copilot/mcp-config.json example.',
  },
  {
    id: 'E3',
    title: '[E3] Coordinator self-update — capture closing summary into squadboard inbox',
    prompt:
      'Verification: note in coordinator playbook that for THIS repo, after work completes, the coordinator should also capture a closing summary into the squadboard inbox so it appears as "done" on its own board. Optional but a nice symmetry.',
  },
  {
    id: 'E4',
    title: '[E4] Heartbeat first-tick verification — capture screenshots',
    prompt:
      'Verification: after B3 ships, watch for one full tick on the Heartbeat page; capture screenshots for the closing handoff.',
  },
];
...
async function main(): Promise<void> {
  ...
  if (!projectId) {
    console.error(
      '[seed] SQUADBOARD_PROJECT_ID is required (the squadboard self-registered project id).',
    );
    process.exit(2);
  }
  ...
  for (const item of ITEMS) {
    ...
    try {
      ...
      if (classification.intent !== 'issue') {
        ...
        console.log(
          `MISCLASSIFIED → forced — intent=${classification.intent} confidence=${classification.confidence.toFixed(2)} card=${forced.id}`,
        );
        results.push({
          id: item.id,
          title: item.title,
      ...
      console.log(
        `OK — issue ${created.id} (intent=issue, confidence=${classification.confidence.toFixed(2)})`,
      );
      results.push({
        id: item.id,
        title: item.title,
  ...
  for (const r of results) {
    console.log(
      `  ${r.id.padEnd(4)} intent=${r.intent.padEnd(6)} conf=${r.confidence.toFixed(2)} ok=${r.ok ? 'yes' : 'no '} card=${r.cardId ?? '-'} ${r.note ? `[${r.note}]` : ''}`,
    );
  }
