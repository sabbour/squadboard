...
import { shouldRun, readConfig, type GuardResult } from './guards.js';
import { Scheduler } from './scheduler.js';
import { invokeCeremony, type InvokerResult } from './invoker.js';
import { gitPush, type PushResult } from './git-push.js';
...
let lastCeremonyResult: InvokerResult | null = null;
let lastGuardResult: GuardResult | null = null;
let lastBackupAt: Date | null = null;
let nextBackupAt: Date = new Date(Date.now() + DEFAULT_BACKUP_INTERVAL_MS);

// ---------------------------------------------------------------------------
// PID file helpers
// ---------------------------------------------------------------------------

function ensureSquadboardDir(): void {
...
function isLiveDaemonRunning(): boolean {
  const pid = readPidFile();
  if (pid === null) return false;
  if (pid === process.pid) return false; // we ARE the daemon
...
function rotateLockIfNeeded(): void {
...
async function maybeRunBackup(tickAt: Date): Promise<void> {
  if (tickAt < nextBackupAt) return;
...
async function executeTick(): Promise<void> {
  ...
  let ceremonyResult: InvokerResult | null = null;
  ...
  console.log(
    `[daemon] tick at ${tickAt.toISOString()}, guards: ${JSON.stringify(guardResult.guards)}, ` +
...
export interface DaemonStartOptions {
  /** If true, fire an initial tick immediately on start. Default false. */
...
export interface DaemonStatus {
  ...
  lastCeremonyResult: InvokerResult | null;
  guards: GuardResult | null;
  backup: {
...
export function startDaemon(opts: DaemonStartOptions = {}): void {
  ...
  if (isLiveDaemonRunning()) {
    const pid = readPidFile();
    console.error(
      `[daemon] refusing to start — a daemon is already running (PID ${pid}). ` +
 ...
 */
export async function testTick(): Promise<void> {
  await executeTick();
}
