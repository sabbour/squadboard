...
async function backupViaHttp(): Promise<{ path: string; sizeBytes: number; sizeMB: number; durationMs: number; pruned: string[] }> {
    ...
    const req = http.request(`${SERVER_BASE}/api/system/backup`, { method: 'POST', headers: { 'Content-Length': '0' } }, (res) => {
      ...
      res.on('end', () => {
        try {
          ...
          else reject(new Error(`Server backup failed: ${body}`));
        } catch (e) {
          reject(e);
        }
      });
...
async function main(): Promise<void> {
  ...
  try {
    ...
    console.error('[backup] ❌ Backup failed:', (err as Error).message);
    process.exit(1);
  }
}
