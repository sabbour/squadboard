...
function parseArgs(argv: string[]): {
  projectId: string;
  file: string;
  statusMap: Map<string, InertStatus>;
  keyPrefix: string;
  dryRun: boolean;
  bodyFooter: string;
  createdBy: string;
  ...
  for (let i = 0; i < args.length; i++) {
    switch (args[i]) {
      ...
      case '--status-map':   statusMapStr = args[++i]; break;
      case '--key-prefix':   keyPrefix   = args[++i]; break;
      case '--dry-run':      dryRun      = true;      break;
      case '--body-footer':  bodyFooter  = args[++i]; break;
      case '--created-by':   createdBy   = args[++i]; break;
  ...
  const statusMap = new Map<string, InertStatus>();
  for (const pair of statusMapStr.split(',')) {
    ...
    statusMap.set(src.trim(), tgt as InertStatus);
  }
...
function mapStatus(src: string | undefined, statusMap: Map<string, InertStatus>): InertStatus {
  if (!src) return statusMap.get('*') ?? 'backlog';
...
async function main() {
  ...
  let inputRows: Array<{ id: string; title: string; description?: string; status?: string }>;
  try {
  ...
  if (opts.json) {
    ...
    console.log(
      `created=${result.created} skipped=${result.skipped} errors=${result.errors.length}${dryTag}`,
    );
  }
