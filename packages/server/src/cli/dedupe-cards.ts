...
function parseArgs(argv: string[]): CliOpts {
  ...
  for (let i = 0; i < args.length; i++) {
    switch (args[i]) {
      ...
      case '--project-id':   projectId = args[++i]; break;
      case '--yes':          yes = true;             break;
      default:
        if (args[i] && !args[i]!.startsWith('-')) break;
        if (args[i]) { console.error(`Unknown flag: ${args[i]}`); process.exit(1); }
    }
  }
...
async function dedupeViaHttp(opts: CliOpts): Promise<DedupeReport> {
  const body = JSON.stringify({ dryRun: opts.dryRun, projectId: opts.projectId ?? undefined });
  return new Promise<DedupeReport>((resolve, reject) => {
    const req = http.request(
      `${SERVER_BASE}/api/system/dedupe`,
      ...
      (res) => {
        ...
        res.on('end', () => {
          try {
            const parsed = JSON.parse(data) as DedupeReport;
            if (!parsed.ok) reject(new Error(`Server dedupe failed: ${data}`));
            else resolve(parsed);
          } catch (e) { reject(e); }
        });
      },
...
async function findDuplicates(projectId: string | null): Promise<Group[]> {
  const pool = getPool();
  ...
  const nodeIdRows = await pool.query<DupRow>(
    `SELECT project_id, github_node_id AS key_value,
...
async function archiveDuplicates(groups: Group[]): Promise<void> {
  const pool = getPool();
  ...
  for (const g of groups) {
    ...
    await pool.query(
      `UPDATE issues SET archived=1, archived_at=$1::timestamptz, archived_reason=$2, updated_at=$1::timestamptz WHERE id IN (${placeholders})`,
...
async function main(): Promise<void> {
    ...
    if (!opts.dryRun && groups.length > 0) {
      ...
      if (!opts.yes) {
        ...
        const answer = await rl.question(
          `About to archive ${totalToArchive} duplicate(s) across ${Object.keys(projects).length} project(s). Proceed? [y/N] `,
        );
        rl.close();
  ...
  if (report.dryRun || !serverLive) {
    ...
    if (report.dryRun) {
      ...
      const total = Object.values(report.projects).reduce((s, p) => s + p.archived, 0);
      if (total === 0) console.error('[dedupe] ✅ No duplicates found — board is clean.');
      else console.error(`[dedupe] ✅ Archived ${total} duplicate(s).`);
    }
  } else {
    process.stdout.write(JSON.stringify(report, null, 2) + '\n');
    const total = Object.values(report.projects).reduce((s, p) => s + p.archived, 0);
