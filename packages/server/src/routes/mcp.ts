...
projectMcpRouter.get('/:id', async (req, res) => {
  try {
    ...
    if (!row) { res.status(404).json({ ok: false, error: 'mcp server not found' }); return; }
...
projectMcpRouter.post('/', async (req, res) => {
  try {
    const { projectId } = req.params as { projectId: string };
    const body = req.body as Partial<mcpService.CreateMcpServerInput>;
...
projectMcpRouter.patch('/:id', async (req, res) => {
  try {
    const { projectId, id } = req.params as { projectId: string; id: string };
    const body = req.body as mcpService.UpdateMcpServerInput;
    ...
    if (!updated) { res.status(404).json({ ok: false, error: 'mcp server not found' }); return; }
...
projectMcpRouter.delete('/:id', async (req, res) => {
  try {
    ...
    if (!ok) { res.status(404).json({ ok: false, error: 'mcp server not found' }); return; }
    res.json({ ok: true });
  } catch (err) { handleError(res, err, 'mcp'); }
});
...
agentMcpRouter.get('/', async (req, res) => {
  try {
    ...
    res.json({ ok: true, data: rows });
  } catch (err) { handleError(res, err, 'agent-mcp'); }
});
...
agentMcpRouter.delete('/:mcpServerId', async (req, res) => {
  try {
    const { agentId, mcpServerId } = req.params as { agentId: string; mcpServerId: string };
    const ok = await mcpService.unassignMcpServerFromAgent(agentId, mcpServerId);
    if (!ok) { res.status(404).json({ ok: false, error: 'assignment not found' }); return; }
    res.json({ ok: true });
  } catch (err) { handleError(res, err, 'agent-mcp'); }
});
