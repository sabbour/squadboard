...
function classifyVendor(id: string): ModelInfo['vendor'] {
...
function classifyTier(id: string, multiplier?: number): ModelInfo['tier'] {
  const lower = id.toLowerCase();
  if (lower.includes('opus')) return 'powerful';
  if (lower.includes('mini') || lower.includes('haiku') || lower.includes('nano')) return 'fast';
  if (typeof multiplier === 'number' && multiplier >= 5) return 'powerful';
  if (typeof multiplier === 'number' && multiplier <= 0.5) return 'fast';
  return 'balanced';
}
...
async function fetchModelsFromSdk(): Promise<ModelInfo[]> {
  ...
  try {
    ...
    const mapped: ModelInfo[] = sdkModels.map((m) => ({
      id: m.id,
      label: m.name ?? m.id,
...
async function getModels(forceRefresh = false): Promise<{ models: ModelInfo[]; source: 'sdk' | 'fallback' | 'cache'; error?: string }> {
  const now = Date.now();
