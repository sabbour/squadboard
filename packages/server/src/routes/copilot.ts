 ...
 */
copilotRouter.post(
  '/:projectId/issues/:issueId/copilot/assign',
  async (req: Request, res: Response) => {
    ...
    try {
      ...
      if (dispatchResult.workflowRunId) externalRef['workflowRunId'] = dispatchResult.workflowRunId;
      if (dispatchResult.runUrl) externalRef['runUrl'] = dispatchResult.runUrl;
      if (dispatchResult.issueNumber) externalRef['issueNumber'] = dispatchResult.issueNumber;
      ...
      const [run] = await db
        ...
        .values({
          ...
          externalRef,
        } as typeof schema.issueRuns.$inferInsert)
        .returning();
 ...
 */
copilotRouter.get('/:projectId/copilot/rules', async (req: Request, res: Response) => {
 ...
 */
copilotRouter.post('/:projectId/copilot/rules', async (req: Request, res: Response) => {
  const { projectId } = req.params as Record<string, string>;
  const { label, enabled = true } = req.body as { label?: unknown; enabled?: unknown };
 ...
 */
copilotRouter.patch('/:projectId/copilot/rules/:id', async (req: Request, res: Response) => {
  ...
  try {
    ...
    const [updated] = await db
      ...
      .where(
        and(
          eq(schema.copilotAutoAssignRules.id, id),
          eq(schema.copilotAutoAssignRules.projectId, projectId),
        ),
      )
      .returning();
 ...
 */
copilotRouter.delete('/:projectId/copilot/rules/:id', async (req: Request, res: Response) => {
  const { projectId, id } = req.params as Record<string, string>;
  try {
    const db = getDb();
    await db
 ...
 */
async function ensureCopilotAgent(
  projectId: string,
): Promise<{ id: string }> {
  const db = getDb();
  const pool = getPool();
  ...
  const [existing] = await db
    ...
    .where(
      and(
        ...
        eq(schema.agents.agentKind, 'copilot'),
      ),
    )
    .limit(1);
...
export async function handleLabeledAutoAssign(params: {
  projectId: string;
  label: string;
  issueId: string | null;        // local Squadboard issue UUID (null if not synced)
  githubIssueNumber: number;
}): Promise<void> {
  const { projectId, label, issueId, githubIssueNumber } = params;

  if (!issueId) return; // can't dispatch without a local issue UUID
  ...
  const rules = await db
    ...
    .where(
      and(
        eq(schema.copilotAutoAssignRules.projectId, projectId),
        eq(schema.copilotAutoAssignRules.label, label),
        eq(schema.copilotAutoAssignRules.enabled, true),
      ),
    );
  ...
  for (const rule of rules) {
    ...
    const guard = await pool.query(
      `SELECT id FROM copilot_auto_assign_dispatches WHERE rule_id = $1 AND issue_id = $2 LIMIT 1`,
      [rule.id, issueId],
    );
    if (guard.rows.length > 0) continue;
    ...
    try {
      ...
      await pool.query(
        `INSERT INTO copilot_auto_assign_dispatches (id, rule_id, issue_id, created_at)
      ...
      console.log(
        `[copilot] auto-dispatched to @copilot for issue ${issueId} (label="${label}", rule=${rule.id})`,
      );
    } catch (err) {
