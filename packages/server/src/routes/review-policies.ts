...
import {
  ...
  loadProjectDefault,
  resolvePolicyForDeliverable,
} from '../services/review-policy-resolver.js';
...
function badRequest(res: Response, error: string, details?: unknown): void {
...
router.get('/presets', async (req: Request, res: Response) => {
  ...
  const rows = await db
    ...
    .where(
      or(
        eq(schema.reviewPolicyPresets.scope, 'system'),
        and(
          eq(schema.reviewPolicyPresets.scope, 'project'),
          eq(schema.reviewPolicyPresets.projectId, projectId),
        ),
      ),
    );
  ...
  const existing = await db
    ...
    .where(
      and(
        eq(schema.reviewPolicyPresets.scope, 'project'),
        eq(schema.reviewPolicyPresets.projectId, projectId),
        eq(schema.reviewPolicyPresets.slug, slug),
      ),
    )
    .limit(1);
...
router.patch('/presets/:slug', async (req: Request, res: Response) => {
  const { projectId, slug } = req.params as Record<string, string>;
  if (!(await ensureProjectExists(projectId))) {
    notFound(res, 'Project not found');
    return;
  }
...
router.delete('/presets/:slug', async (req: Request, res: Response) => {
  const { projectId, slug } = req.params as Record<string, string>;
  if (!(await ensureProjectExists(projectId))) {
    notFound(res, 'Project not found');
    return;
...
router.delete('/default', async (req: Request, res: Response) => {
  const { projectId } = req.params as Record<string, string>;
  if (!(await ensureProjectExists(projectId))) {
    notFound(res, 'Project not found');
    return;
...
router.put('/default/board/:boardId', (_req: Request, res: Response) => {
  badRequest(
    res,
    'board-scope review-policy defaults are not yet supported (boards table not yet shipped)',
  );
});
