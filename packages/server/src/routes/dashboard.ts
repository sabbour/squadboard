...
interface ProjectStats {
  ...
  /** Most recent activity timestamp across runs/sessions/issues. */
  lastActivityAt: string | null;
}

interface ActiveItem {
  kind: 'live-session' | 'issue-run';
  id: string;
  projectId: string;
  projectName: string;
  agentName: string | null;
  /** For live sessions: the session name; for issue runs: the issue title. */
  title: string;
  status: string;
  startedAt: string | null;
  /** Optional model used (for live sessions). */
  model?: string | null;
}

interface RecentActivity {
  kind: 'run-completed' | 'run-failed' | 'session-started' | 'session-ended' | 'comment' | 'issue-created' | 'deliverable';
  ts: string;
  projectId: string;
  projectName: string;
  summary: string;
...
router.get('/', async (_req: Request, res: Response) => {
  try {
    ...
    type ProjectRow = {
      ...
      mtd_spend: string | number;
      last_activity_at: Date | string | null;
    };

    const projects: ProjectStats[] = (projectRows.rows as ProjectRow[]).map((r) => ({
      id: r.id,
      name: r.name,
      path: r.path,
      ...
      monthlyBudgetUsd:
        r.monthly_budget_usd === null || r.monthly_budget_usd === undefined
          ? null
    ...
    const activeNow: ActiveItem[] = [
      ...(liveSessionRows.rows as Array<{
        id: string;
        status: string;
        name: string | null;
    ...
    const recentActivity: RecentActivity[] = (
      recentRows.rows as Array<{
        kind: 'run-completed' | 'issue-created';
        ts: Date | string;
        project_id: string;
        project_name: string;
        status: string | null;
      ...
      const summary =
        r.kind === 'run-completed'
          ? isFailed
            ? `${r.agent_name ?? 'agent'} failed on "${r.title}"`
            : `${r.agent_name ?? 'agent'} completed "${r.title}"`
          : `New issue: "${r.title}"`;
      return {
        kind: isFailed ? ('run-failed' as const) : (r.kind as RecentActivity['kind']),
