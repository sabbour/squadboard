...
vi.mock('../services/formulator.js', () => ({
  ...
  extractJsonObject: vi.fn((raw: string) => {
    try { return JSON.parse(raw); } catch { return null; }
  }),
}));

import { classifyAndDraft, ALL_INTENTS, __test__ } from '../services/conjure-classifier.js';
import type { ConjureIntent } from '../services/conjure-classifier.js';
...
async function classify(prose: string, opts: { hint?: ConjureIntent; useLlm?: boolean } = {}) {
...
describe('LLM degradation path', () => {
  ...
  it('does not throw when LLM is unavailable', async () => {
    await expect(
      classifyAndDraft({ prose: 'help me think through the architecture', useLlm: true }),
...
describe('LLM happy path — candidates parsed from LLM response', () => {
  beforeEach(() => {
    vi.mocked(runFormulator).mockResolvedValue({
      ...
      modelUsed: { model: 'claude-haiku-4.5', via: 'fallback' as const },
    });
  });
...
describe('request field backward compat', () => {
  ...
  it('throws 400 when neither prose nor prompt is provided', async () => {
    await expect(classifyAndDraft({ useLlm: false } as Parameters<typeof classifyAndDraft>[0])).rejects.toMatchObject({
      status: 400,
    });
  });
});
...
describe('hint boosts score for each intent', () => {
  const hintTests: Array<[ConjureIntent, string]> = [
