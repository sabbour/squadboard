...
async function ensureKeyFile(dir: string): Promise<Buffer> {
  ...
  try { await fs.chmod(keyPath, 0o600); } catch { /* best effort */ }
  return fresh;
}

export async function getProjectSecretKey(projectId: string): Promise<Buffer> {
...
export function encryptString(key: Buffer, plaintext: string): EncryptedEnvelope {
...
export function decryptString(key: Buffer, env: EncryptedEnvelope): string {
