...
/**
 * Persisted shape (preset.payload, default.payload, raw YAML form).
 * Snake-case keys mirror the YAML form so `JSON.parse` of YAML output works
 * directly. All fields optional — missing keys inherit from the next layer.
 */
...
/**
 * Resolved (fully-merged) shape. Every field has a value; never undefined.
 * Caller can serialise this into `step_runs.policy_resolved`.
 */
export interface ResolvedReviewPolicy {
  approvers: string[];                     // legacy bare-string list (executable path)
  approver_objects: TypedApprover[];       // typed list (UI / human reviewers)
  request_changes_policy: RequestChangesPolicy;
...
export interface ResolvedReviewPolicyWithSources {
  policy: ResolvedReviewPolicy;
  sources: PolicySources;
  /** Validation findings — empty array means policy is internally consistent. */
  warnings: string[];
}
...
const SYSTEM_DEFAULT: ResolvedReviewPolicy = {
  approvers: [],
  approver_objects: [],
...
export interface ResolveLayers {
  /** Highest-priority override; for steps this comes from the YAML. */
  override?: ReviewPolicyPayload;
  /** Board default (Phase 8 boards work — undefined until boards land). */
  board?: ReviewPolicyPayload;
  /** Project default. */
  project?: ReviewPolicyPayload;
}
 ...
 */
export function resolvePolicyForStep(
  step: ApproveStep,
  layers: Omit<ResolveLayers, 'override'>,
): ResolvedReviewPolicyWithSources {
  return mergePolicy({
 ...
 */
export function resolvePolicyForDeliverable(
  layers: Omit<ResolveLayers, 'override'>,
): ResolvedReviewPolicyWithSources {
...
export function validatePolicyPayload(
  raw: unknown,
): { ok: true; payload: ReviewPolicyPayload } | { ok: false; errors: string[] } {
  const errors: string[] = [];
  if (typeof raw !== 'object' || raw === null) {
    return { ok: false, errors: ['policy payload must be a JSON object'] };
  }
  const r = raw as Record<string, unknown>;
  const out: ReviewPolicyPayload = {};

  if (r['approvers'] !== undefined) {
    if (!Array.isArray(r['approvers']) || (r['approvers'] as unknown[]).some((v) => typeof v !== 'string')) {
      errors.push('approvers must be an array of strings');
    } else {
      out.approvers = (r['approvers'] as string[]).slice();
    }
  }
  ...
  if (r['approver_objects'] !== undefined) {
    if (!Array.isArray(r['approver_objects'])) {
      ...
      const validKinds = new Set<TypedApprover['kind']>(['agent', 'human', 'role']);
      const objs: TypedApprover[] = [];
      for (let i = 0; i < (r['approver_objects'] as unknown[]).length; i++) {
        const entry = (r['approver_objects'] as unknown[])[i];
        if (typeof entry !== 'object' || entry === null) {
          errors.push(`approver_objects[${i}] must be an object`);
          continue;
        }
        const e = entry as Record<string, unknown>;
        const kind = e['kind'] as TypedApprover['kind'];
        const ref = e['ref'];
        if (!validKinds.has(kind)) errors.push(`approver_objects[${i}].kind must be agent | human | role`);
        if (typeof ref !== 'string' || !ref) errors.push(`approver_objects[${i}].ref must be a non-empty string`);
  ...
  if (r['quorum'] !== undefined) {
    ...
    if (typeof q !== 'object' || q === null) {
      ...
      if (!Number.isInteger(n) || n < 1) errors.push('quorum.n must be a positive integer');
      else if (!Number.isInteger(of) || of < n) errors.push('quorum.of must be an integer >= quorum.n');
      else out.quorum = { n, of };
    }
  }

  if (r['exclude_author'] !== undefined) {
    if (typeof r['exclude_author'] !== 'boolean') errors.push('exclude_author must be boolean');
    else out.exclude_author = r['exclude_author'];
  }

  if (r['timeout'] !== undefined) {
    if (typeof r['timeout'] !== 'string' || !r['timeout']) errors.push('timeout must be a non-empty string');
    else out.timeout = r['timeout'];
  }
 ...
 */
export function validateResolvedPolicy(policy: ResolvedReviewPolicy): string[] {
  const warnings: string[] = [];
  if (policy.quorum) {
    const eligible = policy.approver_objects.length || policy.approvers.length;
    if (eligible > 0 && policy.quorum.n > eligible) {
      warnings.push(
        `quorum.n=${policy.quorum.n} exceeds eligible reviewer count (${eligible}); review will never advance`,
      );
    }
    if (eligible > 0 && policy.quorum.of > eligible) {
      warnings.push(
        `quorum.of=${policy.quorum.of} exceeds eligible reviewer count (${eligible})`,
      );
    }
  }
...
export async function loadProjectDefault(projectId: string): Promise<ReviewPolicyPayload | undefined> {
  const db = getDb();
  const rows = await db
    ...
    .where(
      and(
        eq(schema.reviewPolicyDefaults.scope, 'project'),
        eq(schema.reviewPolicyDefaults.scopeId, projectId),
      ),
    )
    .limit(1);
  ...
  if (!validated.ok) {
    console.warn(
      `[review-policy-resolver] project default for ${projectId} failed validation: ${validated.errors.join(', ')}`,
    );
    return undefined;
  }
...
// ---------------------------------------------------------------------------
// Internal: merge layers
// ---------------------------------------------------------------------------

interface InternalLayers {
  override?: ReviewPolicyPayload;
  board?: ReviewPolicyPayload;
  project?: ReviewPolicyPayload;
}

function mergePolicy(layers: InternalLayers): ResolvedReviewPolicyWithSources {
  const policy: ResolvedReviewPolicy = { ...SYSTEM_DEFAULT };
...
function applyLayer(
  policy: ResolvedReviewPolicy,
  sources: PolicySources,
  layer: ReviewPolicyPayload | undefined,
...
function stepToPayload(step: ApproveStep): ReviewPolicyPayload {
  const out: ReviewPolicyPayload = {};
  if (step.approvers !== undefined) out.approvers = step.approvers.slice();
  if (step.approverObjects !== undefined) out.approver_objects = step.approverObjects.slice();
  if (step.request_changes_policy !== undefined) out.request_changes_policy = step.request_changes_policy;
  if (step.quorum !== undefined) out.quorum = { n: step.quorum.n, of: step.quorum.of };
  if (step.exclude_author !== undefined) out.exclude_author = step.exclude_author;
  if (step.timeout !== undefined) out.timeout = step.timeout;
  if (step.timeoutAction !== undefined) out.timeout_action = step.timeoutAction;
  if (step.fallbackReviewer !== undefined) out.fallback_reviewer = step.fallbackReviewer;
  return out;
}
