...
async function tick(): Promise<void> {
  ...
  const byRepo = new Map<string, typeof rows.rows>();
  ...
  for (const [repoKey, runs] of byRepo.entries()) {
    ...
    try {
      ...
      prs = JSON.parse(stdout.trim()) as GhPrEntry[];
    } catch (err) {
    ...
    for (const run of runs) {
      ...
      if (run.pr_state && !['open', 'draft', null].includes(run.pr_state)) continue;

      // Match: headRefName equals git_branch, OR GitHub issue number appears in PR body.
      const matched = prs.find((pr) => {
        if (run.git_branch && pr.headRefName === run.git_branch) return true;
      ...
      if (run.pr_number !== matched.number || run.pr_state !== prState) {
        ...
        if (!run.pr_number || run.pr_number !== matched.number) {
          ...
          console.log(
            `[copilot-watcher] detected PR #${matched.number} for run ${run.run_id} (draft=${matched.isDraft})`,
          );
        }
      }
      ...
      if (!matched.isDraft && ['merged', 'closed'].includes(matched.state.toLowerCase())) {
        await db
          ...
          .where(eq(schema.issueRuns.id, run.run_id));
        console.log(
          `[copilot-watcher] run ${run.run_id} marked completed (PR ${matched.state})`,
        );
      }
    }
  }
...
export function startCopilotWatcher(): void {
  if (_interval) return;
...
export function stopCopilotWatcher(): void {
  if (_interval) {
