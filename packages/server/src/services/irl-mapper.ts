...
import type { LoadedSquadConfig } from '../sdk/squad-config-loader.js';
...
export interface PlannedAgent {
  ...
  role: string;
  model: string | null;
  charterPath: string;          // relative to .squad/
  charterContents: string;
}

export interface PlannedRoutingRule {
  priority: number;
  pattern: string;
...
export interface PlannedCeremony {
  name: string;
  filePath: string;             // relative to .squad/
  contents: string;
}
...
export interface IrlProvisioningPlan {
  defaultModel: string | null;
  teamName: string | null;
  teamMembers: string[];
  agents: PlannedAgent[];
  routingRules: PlannedRoutingRule[];
  ceremonies: PlannedCeremony[];
  // Top-level files written verbatim (e.g. team.md, routing.md).
  files: PlannedFile[];
  warnings: string[];
}
...
// Pure mapping
// ---------------------------------------------------------------------------
export function mapIrlConfigToPlan(loaded: LoadedSquadConfig): IrlProvisioningPlan {
  const warnings = [...loaded.warnings];
  const cfg = (loaded.config ?? {}) as Record<string, unknown>;

  const team = (cfg.team ?? null) as Record<string, unknown> | null;
  const defaults = (cfg.defaults ?? null) as Record<string, unknown> | null;
  const routing = (cfg.routing ?? null) as Record<string, unknown> | null;
  const agentsArr = Array.isArray(cfg.agents) ? (cfg.agents as Array<Record<string, unknown>>) : [];
  const ceremoniesArr = Array.isArray(cfg.ceremonies)
    ? (cfg.ceremonies as Array<Record<string, unknown>>)
    : [];
  ...
  // ---- Routing rules ----
  const routingRules: PlannedRoutingRule[] = [];
  const routingRulesIn = Array.isArray(routing?.rules)
    ? (routing!.rules as Array<Record<string, unknown>>)
    : [];
  ...
  for (let i = 0; i < routingRulesIn.length; i++) {
    ...
    const targetAgent =
      agentsArrRule[0] ?? stripAt(stringOr(r?.assign ?? r?.agent ?? r?.to ?? '', ''));
    if (!targetAgent) {
    ...
    let matchType: PlannedRoutingRule['matchType'];
    let pattern: string;
    if (labels.length) {
      matchType = 'label';
      pattern = labels.join(',');
  ...
  // ---- Ceremonies ----
  const ceremonies: PlannedCeremony[] = ceremoniesArr.map((c, i) => {
...
// ---------------------------------------------------------------------------
// Side-effecting materialiser
// ---------------------------------------------------------------------------
export interface MaterialiseResult {
  agentsInserted: number;
  routingRulesInserted: number;
  filesWritten: number;
  // Phase 11: ceremony narratives create kind='narrative' workflow rows;
  // each is then auto-translated into a draft kind='ceremony' row. A
  // translation failure does NOT abort materialisation — the narrative
  // remains addressable and the user can retry from the review UI.
...
export async function materialiseIrlPlan(
  projectId: string,
  projectPath: string,
  ...
  const translationFailures: MaterialiseResult['translationFailures'] = [];
...
// Renderers
// ---------------------------------------------------------------------------
function renderCharter(a: Record<string, unknown>, role: string): string {
  ...
  lines.push('');
  for (const item of expertiseItems) lines.push(`- ${item}`);
  lines.push('');
  if (charterBody) {
    lines.push('## Style');
    lines.push('');
...
function renderTeam(team: Record<string, unknown>, agents: PlannedAgent[]): string {
...
function renderRoutingMd(rules: PlannedRoutingRule[]): string {
  const lines: string[] = [];
  lines.push('# Routing');
  lines.push('');
...
function renderDefaults(defaults: Record<string, unknown>): string {
  const lines: string[] = [];
  lines.push('# Defaults');
  lines.push('');
  lines.push('```json');
...
function renderCeremony(c: Record<string, unknown>, name: string): string {
  const lines: string[] = [];
  lines.push(`# ${name}`);
  lines.push('');
  if (c?.description) lines.push(stringOr(c.description, ''));
  lines.push('');
  lines.push('```json');
...
function pickModelString(v: unknown): string | null {
  if (typeof v === 'string') return v;
