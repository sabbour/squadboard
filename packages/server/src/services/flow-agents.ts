...
export interface FlowAgentsResponse {
  agents: FlowAgent[];
}
...
export interface FlowLineageResponse {
...
function mapStatus(dbStatus: string): FlowAgentInstance['status'] {
  switch (dbStatus) {
    case 'running':
    case 'active':
    case 'splitting':
...
interface InstanceRow {
  ...
  current_step_started_at: Date | string | null;
}
...
export async function getFlowAgents(projectId: string): Promise<FlowAgentsResponse> {
  const db = getDb();
  ...
  `)).rows as unknown as InstanceRow[];

  // ── 3. Group instances by agent_id ────────────────────────────────────────
  const instancesByAgent = new Map<string, FlowAgentInstance[]>();
  for (const row of instanceRows) {
    ...
    const currentStep: FlowAgentInstance['currentStep'] = row.current_step_id
      ? {
    ...
    instancesByAgent.get(row.agent_id)!.push({
      ...
      instanceKind: row.instance_kind as FlowAgentInstance['instanceKind'],
  ...
  const agents: FlowAgent[] = agentRows.map((agent) => ({
    agentId: agent.id,
    name: agent.name,
    role: agent.role,
...
export async function getFlowLineage(projectId: string): Promise<FlowLineageResponse> {
  const db = getDb();
