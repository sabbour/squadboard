...
export interface FlowReviewEvent {
  ...
  kind: string;     // verb: approve | request_changes | comment | dismiss
...
export interface FlowStepRun {
  id: string;
  stepIndex: number;
  kind: string;        // step_type from db (or 'agent_run' for issue_run-only)
  label: string;
  status: string;
...
function summarise(text: string | null | undefined, max = 200): string | null {
  if (!text) return null;
...
function reviewStateFor(stepType: string, decision: string | null): FlowStepRun['reviewState'] {
  if (stepType !== 'approve') return null;
...
export async function getIssueFlow(projectId: string, issueId: string): Promise<IssueFlow> {
  const db = getDb();
  ...
  if (!issue) {
    const err = new Error('Issue not found in project') as Error & { status: number };
    err.status = 404;
    throw err;
  }
  ...
  const [iw] = await db
    ...
    .innerJoin(
      ...
      eq(schema.issueWorkflows.workflowVersionId, schema.workflowVersions.id),
    )
    .innerJoin(
      schema.workflows,
      eq(schema.workflowVersions.workflowId, schema.workflows.id),
    )
    .where(eq(schema.issueWorkflows.issueId, issueId))
    .limit(1);

  let workflowVersion: FlowWorkflowVersion | null = null;
  ...
  for (const r of issueRunRows) agentIds.add(r.agentId);
  const agentRows = agentIds.size > 0
    ...
    : [];
  const agentById = new Map(agentRows.map((a) => [a.id, a] as const));
  ...
  const deliverablesByStep = new Map<string, FlowDeliverable[]>();
  const deliverablesByRun  = new Map<string, FlowDeliverable[]>();
  for (const d of deliverableRows) {
    const fd: FlowDeliverable = { id: d.id, title: d.title, status: d.status, kind: d.kind };
    if (d.stepRunId) {
  ...
  const reviewRows = (stepRunIds.length > 0 || wfRunIds.length > 0)
    ? await db
        ...
        .where(
          stepRunIds.length > 0 && wfRunIds.length > 0
  ...
  const stepsByWfRun = new Map<string, typeof stepRunRows>();
  ...
  const childWfRunsByParent = new Map<string, typeof wfRuns>();
  ...
  const peerReviewByStepRunId = new Map<string, typeof issueRunRows>();
  ...
  if (stepRunRows.length === 0) {
    ...
    const parentStepIdByChildWfRun = new Map<string, string>();
    for (const parentWf of wfRuns) {
      const children = childWfRunsByParent.get(parentWf.id) ?? [];
      if (children.length === 0) continue;
      const fanOutStep = (stepsByWfRun.get(parentWf.id) ?? []).find(
        (s) => s.stepType === 'fan_out',
      );
      if (!fanOutStep) continue;
      for (const c of children) parentStepIdByChildWfRun.set(c.id, fanOutStep.id);
    }
  ...
  const reviewEvents: FlowReviewEvent[] = reviewRows.map((r) => ({
...
export async function getProjectFlow(projectId: string): Promise<ProjectFlow> {
  const db = getDb();
  ...
  const runRows = runRowsResult.rows as RankedRunRow[];
  const runByIssue = new Map<string, RankedRunRow>(runRows.map((r) => [r.issue_id, r]));
  ...
  const delvRows = delvRowsResult.rows as RankedDelvRow[];
  const delvByIssue = new Map<string, RankedDelvRow>(delvRows.map((d) => [d.issue_id, d]));
  ...
  `);
  const activeRunsCount = ((activeRunsResult.rows as Array<{ n: number }>)[0]?.n ?? 0);
  ...
  `);
  const pendingReviewsCount = ((pendingReviewsResult.rows as Array<{ n: number }>)[0]?.n ?? 0);

  // 6. Build columns
  const columns: ProjectFlowColumn[] = COLUMN_CATALOG.map((c) => ({ ...c, issues: [] }));
  const colBySlug = new Map(columns.map((c) => [c.slug, c] as const));
