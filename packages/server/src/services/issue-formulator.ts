...
function buildIssuePrompt(draft: string, existingLabels: string[]): string {
...
function normalizeIssueDraft(parsed: unknown, existingLabelSet: Set<string>): FormulatedIssueDraft {
  ...
  for (const name of existingLabelSet) existingByLower.set(name.toLowerCase(), name);
...
export async function formulateIssueDraft(
  projectId: string,
  draft: string,
): Promise<FormulateIssueResult> {
  ...
  console.log(
    `[issues] formulating draft (${trimmed.length} chars) with model=${modelUsed.model} (via ${modelUsed.via})`,
  );
